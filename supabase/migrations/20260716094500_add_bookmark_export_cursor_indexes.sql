-- Cover bookmark export keysets with the authenticated owner followed by the
-- UUID cursor. The existing user_id-only indexes still require a sort by id.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bookmark_folders_export_user_id_id
  ON public.bookmark_folders (user_id, id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_post_bookmarks_export_user_id_id
  ON public.post_bookmarks (user_id, id);

-- IF NOT EXISTS checks only the index name. Reject wrong definitions and
...
BEGIN
  FOR v_index_name, v_table, v_columns IN
    ...
    FROM (
      VALUES
        (
          'idx_bookmark_folders_export_user_id_id',
          'public.bookmark_folders'::regclass,
          ARRAY['user_id', 'id']::name[]
        ),
        (
          'idx_post_bookmarks_export_user_id_id',
          'public.post_bookmarks'::regclass,
          ARRAY['user_id', 'id']::name[]
        )
  ...
  LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION
        'bookmark export cursor index % is missing, invalid, or has the wrong definition',
        v_index_name;
    END IF;
  END LOOP;
END
