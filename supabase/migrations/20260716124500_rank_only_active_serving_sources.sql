-- Migration: admit only active serving sources to score inputs
--
-- Census and shadow pipelines are allowed to discover identities and compute
-- experimental metrics, but they must never enter the production score cohort.
-- Source lifecycle state is therefore a hard ranking boundary in both the
-- board and fresh first-party branches.
...
BEGIN
  IF pg_catalog.to_regclass('arena.score_inputs') IS NULL
    ...
    RAISE EXCEPTION 'score-input foundations must exist before enforcing serving isolation';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_catalog.pg_attribute attribute
    WHERE attribute.attrelid = 'arena.sources'::regclass
    ...
    FROM pg_catalog.pg_attribute attribute
    WHERE attribute.attrelid = 'arena.sources'::regclass
      AND attribute.attname = 'serving_mode'
      AND attribute.attnum > 0
      AND NOT attribute.attisdropped
  ) THEN
    RAISE EXCEPTION 'source status and serving_mode are required ranking boundaries';
  END IF;

  IF pg_catalog.to_regrole('service_role') IS NULL THEN
    RAISE EXCEPTION 'service_role must exist before enforcing serving isolation';
  END IF;
END
$preflight$;

CREATE TEMP TABLE score_input_total_before
ON COMMIT DROP
AS
...
-- Only a source explicitly promoted to active serving may supply board rows.
SELECT
...
-- Claimed first-party metrics obey the identical source lifecycle boundary.
SELECT
  COALESCE(s.meta->>'legacy_platform', s.slug)             AS platform,
  CASE WHEN s.product_type = 'spot' THEN 'spot' ELSE 'futures' END AS market_type,
  t.exchange_trader_id                                     AS trader_key,
...
DECLARE
  ...
  v_rows_after bigint;
BEGIN
  SELECT row_count INTO STRICT v_rows_before FROM score_input_total_before;
  SELECT count(*)::bigint INTO v_rows_after FROM arena.score_inputs;

  IF v_rows_after > v_rows_before THEN
    RAISE EXCEPTION 'serving isolation unexpectedly increased score-input rows: % -> %',
