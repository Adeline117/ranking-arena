-- Migration: Add fuzzy search RPC function using pg_trgm similarity
-- Provides typo-tolerant search with weighted scoring:
--   exact match > prefix match > contains > fuzzy (trigram similarity)

CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Set default trigram similarity threshold (lower = more permissive)
SELECT set_limit(0.15);

-- Fuzzy search function that returns ranked results with similarity scores
...
-- Function to get "did you mean" suggestions for zero/low-result queries
CREATE OR REPLACE FUNCTION search_did_you_mean(
  search_query text,
  suggestion_limit int DEFAULT 3
)
RETURNS TABLE(
...
BEGIN
  ...
  WHERE
    ...
    AND (
      similarity(ts.handle, search_query) > 0.2
      OR similarity(ts.source_trader_id, search_query) > 0.2
    )
    AND lower(ts.handle) != lower(trim(search_query))
  ORDER BY lower(ts.handle), similarity_score DESC
  LIMIT suggestion_limit;
END;
$$;

-- Add click_through tracking columns to search_analytics
