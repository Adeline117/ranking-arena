-- Migration: expire stale trader_stats fallbacks from score inputs
--
-- A current board row may be sparse. The previous view filled its missing
-- fields from trader_stats without checking that row's age, then exposed the
-- board scraped_at as the freshness of the mixed result. Old profile metrics
-- could therefore look current and influence scoring indefinitely. Keep the
-- board identity/headlines, but admit stats fallbacks for at most 48 hours and
-- expose the oldest observation time used by the mixed row.

BEGIN;

SET TRANSACTION ISOLATION LEVEL REPEATABLE READ;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '2min';

DO $preflight$
BEGIN
  IF pg_catalog.to_regclass('arena.score_inputs') IS NULL
     OR pg_catalog.to_regclass('arena.sources') IS NULL
     OR pg_catalog.to_regclass('arena.leaderboard_snapshots') IS NULL
     OR pg_catalog.to_regclass('arena.leaderboard_entries') IS NULL
     OR pg_catalog.to_regclass('arena.traders') IS NULL
     OR pg_catalog.to_regclass('arena.trader_stats') IS NULL THEN
    RAISE EXCEPTION 'score-input foundations must exist before expiring stale fallbacks';
  END IF;

  IF pg_catalog.to_regrole('service_role') IS NULL THEN
    RAISE EXCEPTION 'service_role must exist before expiring stale fallbacks';
  END IF;
END
$preflight$;

CREATE TEMP TABLE score_input_freshness_before
ON COMMIT DROP
AS
SELECT 'arena.score_inputs'::regclass::oid AS view_oid,
       count(*)::bigint AS row_count
FROM arena.score_inputs;

-- CREATE OR REPLACE must not exchange one trader/source/window for another.
-- Snapshot both the complete view and the exact identity contract used by the
-- production RPC defaults. EXCEPT ALL below also catches duplicate changes.
CREATE TEMP TABLE score_input_membership_before
ON COMMIT DROP
AS
SELECT 'view'::text AS surface,
       platform,
       "window",
       trader_key,
       board_rank
  FROM arena.score_inputs
UNION ALL
SELECT 'rpc48'::text AS surface,
       platform,
       "window",
       trader_key,
       board_rank
  FROM arena.score_inputs
 WHERE (board_rank IS NULL OR board_rank <= 1000)
   AND as_of > now() - interval '48 hours';

CREATE OR REPLACE VIEW arena.score_inputs AS
WITH latest_passed AS (
  SELECT DISTINCT ON (ls.source_id, ls.timeframe)
         ls.id AS snapshot_id, ls.source_id, ls.timeframe, ls.scraped_at
    FROM arena.leaderboard_snapshots ls
   WHERE ls.count_check_passed
   ORDER BY ls.source_id, ls.timeframe, ls.scraped_at DESC
),
fp_fresh AS (
  SELECT st.trader_id, st.timeframe
    FROM arena.trader_stats st
    JOIN arena.traders t ON t.id = st.trader_id
   WHERE t.meta->>'claimed' = 'true'
     AND st.extras->>'provenance' = 'first_party'
     AND st.as_of > now() - interval '48 hours'
)
-- Board membership and headline values belong to the latest passed snapshot.
-- The LEFT JOIN deliberately disappears after 48h: unknown is NULL, never an
-- old value relabelled with lp.scraped_at. The strict > boundary is deliberate:
-- an observation at exactly 48h is expired. This is a conservative row-level
-- gate until individual score fields carry their own observation timestamps.
SELECT
  COALESCE(s.meta->>'legacy_platform', s.slug)             AS platform,
  CASE WHEN s.product_type = 'spot' THEN 'spot' ELSE 'futures' END AS market_type,
  t.exchange_trader_id                                     AS trader_key,
  (lp.timeframe::text || 'D')                              AS "window",
  e.rank                                                   AS board_rank,
  CASE
    WHEN COALESCE(e.headline_roi, st.roi) IS NULL THEN NULL::numeric
    ELSE LEAST(GREATEST(COALESCE(e.headline_roi, st.roi), -10000), 10000)
  END                                                      AS roi_pct,
  COALESCE(e.headline_pnl, st.pnl)                         AS pnl_usd,
  CASE
    WHEN COALESCE(e.headline_win_rate, st.win_rate) IS NULL THEN NULL::numeric
    ELSE LEAST(GREATEST(COALESCE(e.headline_win_rate, st.win_rate), 0), 100)
  END                                                      AS win_rate,
  CASE
    WHEN st.mdd IS NULL THEN NULL::numeric
    ELSE LEAST(abs(st.mdd), 100)
  END                                                      AS max_drawdown,
  st.copier_count                                          AS copiers,
  st.total_positions                                       AS trades_count,
  st.sharpe                                                AS sharpe_ratio,
  (st.extras->>'sortino')::numeric                         AS sortino_ratio,
  (st.extras->>'calmar')::numeric                          AS calmar_ratio,
  (st.extras->>'volatility')::numeric                      AS volatility_pct,
  t.trader_kind,
  t.nickname                                               AS handle,
  COALESCE(t.avatar_url_mirror, t.avatar_url_origin)       AS avatar_url,
  s.currency,
  CASE
    WHEN st.trader_id IS NULL THEN lp.scraped_at
    ELSE LEAST(lp.scraped_at, st.as_of)
  END                                                      AS as_of
FROM latest_passed lp
JOIN arena.sources s             ON s.id = lp.source_id
JOIN arena.leaderboard_entries e ON e.snapshot_id = lp.snapshot_id
JOIN arena.traders t             ON t.id = e.trader_id
LEFT JOIN arena.trader_stats st
  ON st.trader_id = t.id
 AND st.timeframe = lp.timeframe
 AND st.as_of > now() - interval '48 hours'
WHERE s.serving_mode = 'serving'
  AND s.status = 'active'
  AND s.currency = ANY (ARRAY['USDT','USDx','USDC','USD'])
  AND (s.meta->>'legacy_platform') IS DISTINCT FROM 'null'
  AND NOT EXISTS (
    SELECT 1
      FROM fp_fresh f
     WHERE f.trader_id = t.id
       AND f.timeframe = lp.timeframe
  )

UNION ALL

-- Claimed first-party metrics already have a strict 48h admission gate and
-- keep their own observation time.
SELECT
  COALESCE(s.meta->>'legacy_platform', s.slug)             AS platform,
  CASE WHEN s.product_type = 'spot' THEN 'spot' ELSE 'futures' END AS market_type,
  t.exchange_trader_id                                     AS trader_key,
  (st.timeframe::text || 'D')                              AS "window",
  br.rank                                                  AS board_rank,
  CASE
    WHEN st.roi IS NULL THEN NULL::numeric
    ELSE LEAST(GREATEST(st.roi, -10000), 10000)
  END                                                      AS roi_pct,
  st.pnl                                                   AS pnl_usd,
  CASE
    WHEN st.win_rate IS NULL THEN NULL::numeric
    ELSE LEAST(GREATEST(st.win_rate, 0), 100)
  END                                                      AS win_rate,
  CASE
    WHEN st.mdd IS NULL THEN NULL::numeric
    ELSE LEAST(abs(st.mdd), 100)
  END                                                      AS max_drawdown,
  st.copier_count                                          AS copiers,
  st.total_positions                                       AS trades_count,
  st.sharpe                                                AS sharpe_ratio,
  (st.extras->>'sortino')::numeric                         AS sortino_ratio,
  (st.extras->>'calmar')::numeric                          AS calmar_ratio,
  (st.extras->>'volatility')::numeric                      AS volatility_pct,
  t.trader_kind,
  t.nickname                                               AS handle,
  COALESCE(t.avatar_url_mirror, t.avatar_url_origin)       AS avatar_url,
  s.currency,
  st.as_of                                                 AS as_of
FROM arena.traders t
JOIN arena.sources s       ON s.id = t.source_id
JOIN arena.trader_stats st ON st.trader_id = t.id AND st.timeframe IN (7, 30, 90)
LEFT JOIN LATERAL (
  SELECT e2.rank
    FROM latest_passed lp2
    JOIN arena.leaderboard_entries e2
      ON e2.snapshot_id = lp2.snapshot_id
     AND e2.trader_id = t.id
   WHERE lp2.source_id = s.id
     AND lp2.timeframe = st.timeframe
   LIMIT 1
) br ON true
WHERE s.serving_mode = 'serving'
  AND s.status = 'active'
  AND t.meta->>'claimed' = 'true'
  AND st.extras->>'provenance' = 'first_party'
  AND st.as_of > now() - interval '48 hours'
  AND s.currency = ANY (ARRAY['USDT','USDx','USDC','USD']);

GRANT SELECT ON arena.score_inputs TO service_role;

DO $postflight$
DECLARE
  v_oid_before oid;
  v_rows_before bigint;
  v_oid_after oid;
  v_rows_after bigint;
BEGIN
  SELECT view_oid, row_count
    INTO STRICT v_oid_before, v_rows_before
    FROM score_input_freshness_before;
  SELECT 'arena.score_inputs'::regclass::oid, count(*)::bigint
    INTO v_oid_after, v_rows_after
    FROM arena.score_inputs;

  IF v_oid_after IS DISTINCT FROM v_oid_before THEN
    RAISE EXCEPTION 'score_inputs view identity changed: % -> %', v_oid_before, v_oid_after;
  END IF;

  IF v_rows_after IS DISTINCT FROM v_rows_before THEN
    RAISE EXCEPTION 'stale-fallback expiry changed score-input rows: % -> %',
      v_rows_before, v_rows_after;
  END IF;

  IF EXISTS (
    WITH score_input_membership_after AS (
      SELECT 'view'::text AS surface,
             platform,
             "window",
             trader_key,
             board_rank
        FROM arena.score_inputs
      UNION ALL
      SELECT 'rpc48'::text AS surface,
             platform,
             "window",
             trader_key,
             board_rank
        FROM arena.score_inputs
       WHERE (board_rank IS NULL OR board_rank <= 1000)
         AND as_of > now() - interval '48 hours'
    )
    SELECT 1
      FROM (
        (SELECT surface, platform, "window", trader_key, board_rank
           FROM score_input_membership_before
         EXCEPT ALL
         SELECT surface, platform, "window", trader_key, board_rank
           FROM score_input_membership_after)
        UNION ALL
        (SELECT surface, platform, "window", trader_key, board_rank
           FROM score_input_membership_after
         EXCEPT ALL
         SELECT surface, platform, "window", trader_key, board_rank
           FROM score_input_membership_before)
      ) membership_delta
  ) THEN
    RAISE EXCEPTION 'stale-fallback expiry changed view or default RPC48 membership';
  END IF;

  IF NOT pg_catalog.has_table_privilege('service_role', 'arena.score_inputs', 'SELECT') THEN
    RAISE EXCEPTION 'service_role lost score_inputs SELECT access';
  END IF;
END
$postflight$;

NOTIFY pgrst, 'reload schema';

COMMIT;
