...
-- Fills NULL pnl_usd in trader_snapshots_v2 from sibling "window"s of the same trader.
...
BEGIN
  -- Fill NULL pnl_usd from sibling "window"s (same platform + trader_key, different "window")
...
COMMENT ON FUNCTION public.fill_null_pnl_from_siblings IS 'Fill NULL pnl_usd from sibling "window"s for same trader. Used by compute-leaderboard cron.';
