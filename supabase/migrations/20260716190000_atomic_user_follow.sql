-- Move user follow/unfollow behind one service-only transaction boundary.
-- The route must deploy only after this expand migration. Direct table writes
-- remain temporarily available to service_role for the legacy block route;
-- 20260716192000 closes that compatibility window after both routes use RPCs.
...
SELECT pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('public.user_follows:atomic-mutation:v1', 0)
);
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION
      'user follow authority tables must exist before atomic follow deployment';
  END IF;
  ...
  IF v_missing_roles IS NOT NULL THEN
    RAISE EXCEPTION 'user follow ACL roles are missing: %', v_missing_roles;
  END IF;

  IF pg_catalog.to_regprocedure('auth.role()') IS NULL THEN
    RAISE EXCEPTION 'auth.role() is required for the service-only follow RPC';
  END IF;
  ...
  IF v_invalid_columns IS NOT NULL THEN
    RAISE EXCEPTION
      'user follow schema has missing or incompatible columns: %',
      v_invalid_columns;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE index_metadata.indrelid = 'public.user_follows'::pg_catalog.regclass
    ...
    RAISE EXCEPTION
      'user_follows requires an exact valid ordered-pair unique key';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_catalog.pg_index AS index_metadata
    WHERE index_metadata.indrelid = 'public.blocked_users'::pg_catalog.regclass
      AND index_metadata.indisunique
      AND index_metadata.indisvalid
      AND index_metadata.indisready
      AND NOT index_metadata.indisexclusion
    ...
    RAISE EXCEPTION
      'blocked_users requires an exact valid ordered-pair unique key';
  END IF;

  IF v_pair_function IS NULL THEN
    RAISE EXCEPTION
      'canonical block/follow pair serializer must exist before follow RPC';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE trigger_row.tgrelid = 'public.blocked_users'::pg_catalog.regclass
      ...
      AND (
        SELECT pg_catalog.array_agg(
          attribute.attname ORDER BY trigger_column.ordinality
        )
  ...
  IF EXISTS (
    ...
    WHERE namespace_row.nspname = 'public'
      AND function_row.proname = 'mutate_user_follow_atomic'
      AND pg_catalog.pg_get_function_identity_arguments(function_row.oid)
            <> 'p_actor_id uuid, p_target_id uuid, p_action text'
  ) THEN
    RAISE EXCEPTION 'incompatible mutate_user_follow_atomic overload exists';
  END IF;
END
$preflight$;

CREATE OR REPLACE FUNCTION public.mutate_user_follow_atomic(
  p_actor_id uuid,
  p_target_id uuid,
  p_action text
)
...
BEGIN
  IF COALESCE(NULLIF(auth.role(), ''), '') IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'service role required for atomic user follow mutation';
  END IF;

  IF p_actor_id IS NULL
     OR p_target_id IS NULL
     OR p_action IS NULL
     OR p_action NOT IN ('follow', 'unfollow')
  THEN
  ...
  -- Parent identities are locked before the pair key. Account deletion takes
  -- the same Auth parents before its FK cascades reach the pair serializer, so
  -- this order cannot form an Auth-row <-> pair-key cycle.
  ...
  -- All operations touching a shared profile acquire profile rows in UUID
  -- order. This serializes absolute recounts across different follow pairs.
  PERFORM profile.id
  ...
  IF p_action = 'follow' AND EXISTS (
    SELECT 1
    FROM public.blocked_users AS block_edge
    WHERE (
      block_edge.blocker_id = p_actor_id
      AND block_edge.blocked_id = p_target_id
    ) OR (
      block_edge.blocker_id = p_target_id
      AND block_edge.blocked_id = p_actor_id
    )
  ) THEN
  ...
  IF p_action = 'follow' THEN
    INSERT INTO public.user_follows AS follow_edge (
      follower_id,
      following_id
    )
    ...
    v_status := CASE WHEN v_changed THEN 'followed' ELSE 'already_following' END;
  ELSE
    ...
    GET DIAGNOSTICS v_actor_following_count = ROW_COUNT;
    v_changed := v_actor_following_count = 1;
    v_status := CASE WHEN v_changed THEN 'unfollowed' ELSE 'already_not_following' END;
  END IF;
  ...
  RETURN pg_catalog.jsonb_build_object(
    ...
    'followed_by', v_followed_by,
    'mutual', v_following AND v_followed_by,
...
ALTER FUNCTION public.mutate_user_follow_atomic(uuid, uuid, text)
  OWNER TO postgres;
...
COMMENT ON FUNCTION public.mutate_user_follow_atomic(uuid, uuid, text) IS
  'Service-only idempotent follow/unfollow boundary with current account/block checks, shared pair serialization, and absolute counter repair.';

DO $postflight$
DECLARE
  v_function regprocedure := 'public.mutate_user_follow_atomic(uuid,uuid,text)'::regprocedure;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'atomic user follow function metadata drifted';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'atomic user follow EXECUTE boundary drifted';
  END IF;
END
$postflight$;
