...
import type { TraderData, DataSource, TimeRange, TraderSourceRow, TraderSnapshotRow } from './types.js'
 ...
 * 保存交易员数据到数据库
 */
export async function saveTraders(
  traders: TraderData[],
  source: DataSource,
  timeRange: TimeRange
  ...
  for (const trader of traders) {
    try {
      ...
      const sourceRow: TraderSourceRow = {
        source,
      ...
      // Insert snapshot
      const snapshotRow: TraderSnapshotRow = {
        source,
        source_trader_id: trader.traderId,
        season_id: timeRange,
        rank: trader.rank,
