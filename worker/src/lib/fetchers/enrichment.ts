...
export async function fetchOkxCurrentPositions(
  ...
  try {
    ...
    return data.data.map((p) => ({
      ...
      direction: (p.posSide || '').toLowerCase().includes('short') ? 'short' : 'long',
      positionType: 'perpetual',
...
interface HyperliquidFill {
  ...
  time?: number
  dir?: string // 'Buy' / 'Sell' / 'Open Long' / 'Close Long' / 'Open Short' / 'Close Short'
  closedPnl?: string
  hash?: string
  oid?: number
  crossed?: boolean
...
export async function fetchHyperliquidPositionHistory(
  ...
  try {
    ...
    // Filter to closing fills (with non-zero closedPnl) and convert to positions
    ...
    return closingFills.map((f) => {
      ...
      // If closing a long (sell), the position was long. If closing a short (buy), it was short.
      ...
      return {
        ...
        openTime: null, // Fills don't have open time
        closeTime: f.time ? new Date(f.time).toISOString() : null,
        entryPrice: null, // Not available in fills
...
// ============================================
// GMX Position History (from GraphQL tradeActions)
// ============================================
...
export async function fetchGmxPositionHistory(
  ...
  try {
    ...
    // Filter to actions with PnL (closing trades)
    ...
    return closingActions.map((a) => {
      ...
      const price = a.executionPrice ? Number(BigInt(a.executionPrice)) / 1e24 : null // GMX prices are 1e24 scale
