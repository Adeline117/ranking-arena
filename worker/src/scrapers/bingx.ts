/**
 * BingX Copy Trading 爬虫
 * 使用 Playwright 浏览器自动化 (SSR + Cloudflare)
 */
...
const PERIOD_CONFIG: Record<TimeRange, { tabTexts: string[] }> = {
  '7D': { tabTexts: ['7天', '7 Days', '7D'] },
  '30D': { tabTexts: ['30天', '30 Days', '30D'] },
  '90D': { tabTexts: ['90天', '90 Days', '90D', '全部', 'All'] },
}

export class BingxScraper extends BaseScraper {
  private readonly baseUrl = 'https://bingx.com/en/copy-trading/'
  ...
  protected async scrapeData(timeRange: TimeRange): Promise<TraderData[]> {
    ...
    this.log.info('Starting BingX scrape', {
      timeRange,
  ...
  private parseTrader(item: Record<string, unknown>, rank: number): TraderData | null {
    ...
    const nickname = String(
      item.nickName || item.nickname || item.name || item.traderName || ''
    )
    if (!nickname) return null

    let roi = parseFloat(String(item.roi ?? item.roiRate ?? item.yieldRate ?? item.profitRate ?? 0))
    ...
    let maxDrawdown = parseFloat(String(item.maxDrawdown ?? item.mdd ?? item.drawdown ?? 0))
    ...
    return {
      traderId,
      nickname,
      avatar: String(item.avatar || item.avatarUrl || item.headUrl || '') || null,
      roi,
      pnl: parseFloat(String(item.pnl ?? item.profit ?? item.totalProfit ?? 0)),
      winRate: winRate || null,
      maxDrawdown: maxDrawdown || null,
      followers: parseInt(String(item.followers ?? item.followerCount ?? item.copyCount ?? 0), 10),
      aum: null,
      tradesCount: null,
      rank,
    }
  ...
  private async extractFromDom(): Promise<TraderData[]> {
    const pageTraders = await this.page!.evaluate(() => {
      ...
      cards.forEach((card) => {
        ...
        const traderId = idMatch?.[1] || `bingx_${Date.now()}_${results.length}`
