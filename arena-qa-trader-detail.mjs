...
async function main() {
  ...
  const sections = {
    ...
    'Win Rate':
      bodyText.includes('Win Rate') || bodyText.includes('Win%') || bodyText.includes('Win'),
    ...
    'Exchange badge':
      bodyText.includes('Hyperliquid') ||
      bodyText.includes('hyperliquid') ||
      bodyText.includes('On-chain'),
  }
    ...
    for (const btn of buttons) {
      ...
      if (isVisible && text && text.trim() === period) {
        ...
        await page.screenshot({
          path: `/tmp/arena-qa-08-trader-${period.toLowerCase()}.png`,
          fullPage: false,
        })
