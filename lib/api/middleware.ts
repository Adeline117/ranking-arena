...
function getSafeErrorMessage(message: string, status: number): string {
  ...
  const suspicious =
    /\b(at |Error:|ENOENT|ECONNREFUSED|\.ts:|\.js:|SELECT |INSERT |UPDATE |DELETE |FROM |WHERE |supabase|postgres|redis|node_modules)\b/i
  ...
  return async (request: NextRequest): Promise<NextResponse> => {
    ...
    return runWithCorrelationId(correlationId, async () => {
      ...
      try {
        // 1. 限流检查
        if (rateLimit !== false) {
          const config = typeof rateLimit === 'string' ? RateLimitPresets[rateLimit] : rateLimit
        ...
        if (duration >= 3000) {
          logger.error(`CRITICAL SLOW API: ${name} took ${duration}ms`, {
            path: request.nextUrl.pathname,
            correlationId,
          })
        } else if (duration >= 1000) {
          logger.warn(`Slow API: ${name} took ${duration}ms`, {
            path: request.nextUrl.pathname,
            correlationId,
          })
        }
        ...
        if (statusCode >= 500) {
          logger.error(`${name} error: ${internalMessage}`, {
            error: String(error),
            duration,
            correlationId,
          })
...
export function withAuth<T>(
  ...
  return withApiMiddleware(handler as ApiHandler<T>, { ...options, requireAuth: true })
}
