...
export const ARENA_V4_CONFIG = {
  // 权重(owner 2026-07-08 定稿,回测校准后):赚钱 50% + 实力 50%。
  // 回测(trader_series 30d 前瞻)证明过去 ROI/PnL 不预测未来收益(均值回归),
  // 故赚钱从 0.70 降到 0.50 上限:PnL 0.28(防刷锚+身份)+ ROI 0.22(跟单者回报);
  // 实力 0.50:回撤 0.20(最高,护跟单者)+ Sharpe 0.16 + 一致性 0.14。
  W: { pnl: 0.28, roi: 0.22, dd: 0.2, sharpe: 0.16, con: 0.14 },
