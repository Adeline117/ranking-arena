    ...
    getSharedRedis()
      .then(redis => {
        ...
        return redis.decr(this.concurrencyKey).then(val => {
          ...
          if (val != null && val < 0) {
            redis.set(this.concurrencyKey, 0, { ex: 30 }).catch(err => dataLogger.warn(`[RedisRateLimiter] Failed to reset concurrency key ${this.concurrencyKey}:`, err instanceof Error ? err.message : String(err)))
          }
        })
      })
      .catch(() => {
