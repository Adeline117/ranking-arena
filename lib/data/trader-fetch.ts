/**
 * Data fetching functions for trader data.
 * Handles profile, performance, stats, portfolio, position history, and feed.
 */
      ...
      try {
        ...
        const [snapshot90d, snapshot7d, snapshot30d] = await Promise.all([
          supabase.from('trader_snapshots')
            ...
            .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
            .eq('season_id', '90D').order('captured_at', { ascending: false }).limit(1).maybeSingle(),
          supabase.from('trader_snapshots')
            .select('roi, pnl, win_rate, max_drawdown')
            .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
            .eq('season_id', '7D').order('captured_at', { ascending: false }).limit(1).maybeSingle(),
          supabase.from('trader_snapshots')
            .select('roi, pnl, win_rate, max_drawdown')
            .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
            .eq('season_id', '30D').order('captured_at', { ascending: false }).limit(1).maybeSingle(),
        ])
...
export async function getTraderStats(handle: string): Promise<TraderStats> {
  try {
    const source = await findTraderAcrossSources(handle)
    if (!source) return { additionalStats: {} }
    ...
    if (snapshots.length === 0) return { additionalStats: {} }
...
export async function getTraderFrequentlyTraded(handle: string): Promise<Array<{
  symbol: string; weightPct: number; count: number; avgProfit: number; avgLoss: number; profitablePct: number
}>> {
  try {
    const source = await findTraderAcrossSources(handle)
    if (!source) return []

    const { data: latestSnapshot } = await supabase.from('trader_snapshots')
      .select('captured_at')
      .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
      .order('captured_at', { ascending: false }).limit(1).maybeSingle()
    ...
    const { data } = await supabase.from('trader_frequently_traded')
      .select('symbol, weight_pct, trade_count, avg_profit, avg_loss, profitable_pct')
      .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
...
export async function getTraderMonthlyPerformance(handle: string): Promise<Array<{ month: string; value: number }>> {
  try {
    ...
    const { data } = await supabase.from('trader_monthly_performance')
      .select('year, month, roi')
      .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
      .order('year', { ascending: false }).order('month', { ascending: false }).limit(12)
...
export async function getTraderYearlyPerformance(handle: string): Promise<Array<{ year: number; value: number }>> {
  try {
    ...
    const { data } = await supabase.from('trader_yearly_performance')
      .select('year, roi')
      .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
      .order('year', { ascending: false }).limit(5)
...
export async function getTraderPortfolio(handle: string): Promise<PortfolioItem[]> {
  try {
    ...
    const { data } = await supabase.from('trader_portfolio')
    ...
    return data.map((item: {
      symbol: string | null; direction: string | null; weight_pct: number | null
      entry_price: number | null; pnl_pct: number | null
    }) => ({
      market: item.symbol || '',
...
export async function getTraderPositionHistory(handle: string): Promise<PositionHistoryItem[]> {
  try {
    ...
    const { data } = await supabase.from('trader_position_history')
      .select('symbol, direction, entry_price, exit_price, pnl_pct, open_time, close_time')
      .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
      .order('close_time', { ascending: false }).limit(50)

    if (!data) return []

    return data.map((item: {
      symbol: string | null; direction: string | null; entry_price: number | null
      exit_price: number | null; pnl_pct: number | null; open_time: string | null; close_time: string | null
    }) => ({
      symbol: item.symbol || '',
  ...
  try {
    ...
    if (userProfile?.id) {
      const { data } = await supabase.from('reposts')
        .select(`id, comment, created_at, post_id, posts (id, title, content, author_handle, group_id, like_count, groups (name))`)
...
export async function getSimilarTraders(handle: string, limit: number = 6): Promise<TraderProfile[]> {
  try {
    ...
    const { data: currentRoiData } = await supabase.from('trader_snapshots')
      .select('roi')
      .eq('source', source.source).eq('source_trader_id', source.source_trader_id)
      .order('captured_at', { ascending: false }).limit(1).maybeSingle()
    ...
    let { data: snapshots } = await supabase.from('trader_snapshots')
      .select('source_trader_id, roi')
      .eq('source', source.source).eq('captured_at', latestSnapshot.captured_at)
      .neq('source_trader_id', source.source_trader_id)
      .gte('roi', minRoi).lte('roi', maxRoi).limit(50)

    if (!snapshots || snapshots.length === 0) {
      const { data: fallback } = await supabase.from('trader_snapshots')
        .select('source_trader_id, roi')
        .eq('source', source.source).eq('captured_at', latestSnapshot.captured_at)
        .neq('source_trader_id', source.source_trader_id)
        .order('roi', { ascending: false }).limit(limit)
      snapshots = fallback
    }
    ...
    const [sourcesResult, followersMap] = await Promise.all([
      supabase.from('trader_sources')
        ...
        .eq('source', source.source).in('source_trader_id', traderIds),
