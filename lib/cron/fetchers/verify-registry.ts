/**
 * Verify Registry -- Maps every platform to a health-check verifier function.
 *
 * Each verifier makes a single minimal API call (page 1, size 1) to check
 * whether the exchange API is reachable and returning valid data.
 *
 * Used by the verify-fetchers cron job for proactive alerting.
 */

import { type VerifyResult, verifyEndpoint } from './shared'
...
const VERIFY_REGISTRY: Record<string, () => Promise<VerifyResult>> = {
  // -- CEX Futures --
    ...
    verifyEndpoint(
      ...
      'https://www.okx.com/api/v5/copytrading/public-lead-traders?instType=SWAP&page=1',
      {
        ...
        validateResponse: (d: any) =>
          ...
          d.data.length > 0 &&
          Array.isArray(d.data[0]?.ranks) &&
          d.data[0].ranks.length > 0,
      }
    ),
  ...
  bitget_futures: () =>
    verifyEndpoint(
      ...
      'https://api.bitget.com/api/v2/copy/mix-trader/trader-profit-ranking?period=THIRTY_DAYS&pageNo=1&pageSize=1',
      {
        ...
        validateResponse: (d: any) => {
          return d?.code === '00000' || d?.code === 0 || d?.code === '0' || !!d?.data
        },
      }
    ),
  ...
  // kwenta, synthetix, mux require THEGRAPH_API_KEY -- verify only if key is set
  kwenta: () => {
    ...
    if (!apiKey) {
      return Promise.resolve({
        ...
        healthy: false,
        apiReachable: false,
        responseFormat: 'error' as const,
        failureReason: 'auth_required' as const,
        latencyMs: 0,
  ...
  // Stub platforms (no public API) -- skip bitfinex, whitebit, btse, vertex
}
...
export function getVerifier(platform: string): (() => Promise<VerifyResult>) | undefined {
...
/**
 * Run all verifiers in batches to avoid overwhelming networks.
 * Returns results for every registered platform.
 */
