...
export interface PaginationConfig {
  type: 'page_number' | 'offset' | 'cursor' | 'none'
  pageSize: number
  maxPages: number
  target: number
  delayMs?: number
    ...
    try {
      for (const period of periods) {
        ...
        if (periods.indexOf(period) < periods.length - 1) {
          await sleep(config.periodDelayMs ?? 1000)
        }
      }
    } catch (err) {
      captureException(err instanceof Error ? err : new Error(String(err)), {
        tags: { platform: config.source },
      })
      logger.error(
        `[${config.source}] Fetch failed`,
...
async function fetchPeriod(
  ...
  if (!exchangePeriod) {
    return { total: 0, saved: 0, error: `No period mapping for ${period}` }
  }
  ...
  if (config.pagination.type === 'none') {
    const data = await fetchPage(config, exchangePeriod, 1, config.pagination.pageSize)
    if (data) {
      if (config.validateResponse && !config.validateResponse(data)) {
        return { total: 0, saved: 0, error: 'Response validation failed' }
      }
      const items = config.mapping.extractList(data)
      for (const item of items) {
        const mapped = config.mapping.mapItem(item, allItems.size)
        if (mapped) allItems.set(mapped.source_trader_id, item)
      }
    }
  } else {
    for (let page = 1; page <= config.pagination.maxPages; page++) {
      ...
      if (!data) {
        lastError = 'API returned no data'
        break
      }
  ...
  for (const [, item] of allItems) {
    ...
    if (config.mapping.roiIsDecimal) {
      roi = Math.abs(roi) < 10 ? roi * 100 : roi
    }

    const minRoi = config.mapping.minRoi ?? 0
    if (roi <= minRoi) continue

    let winRate = mapped.win_rate ?? null
    if (winRate != null && config.mapping.winRateIsDecimal) {
...
async function fetchPage(
  ...
  try {
    ...
    logger.warn(
      `[${config.source}] Page ${page} failed: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
