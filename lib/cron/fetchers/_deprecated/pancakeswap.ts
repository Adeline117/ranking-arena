/**
 * PancakeSwap V3 (BSC) — Inline fetcher for Vercel serverless
 * Fetches top traders from PancakeSwap subgraph via The Graph
 */
...
import { PANCAKESWAP_SUBGRAPH } // @ts-expect-error deprecated module
...
async function fetchSwapsPage(first: number, skip: number): Promise<SwapEntry[]> {
  ...
  const data = await fetchJson<{ data: { swaps: SwapEntry[] } }>(PANCAKESWAP_SUBGRAPH, {
    method: 'POST',
