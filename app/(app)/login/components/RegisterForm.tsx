...
export default function RegisterForm({
  ...
  // Floor: minimum 8 chars AND strength at least "fair" (level >= 2). The strength
  // meter is the real gate — short or weak passwords cannot be submitted.
  const PASSWORD_MIN_LENGTH = 8
  const passwordMeetsFloor = password.length >= PASSWORD_MIN_LENGTH && passwordStrength.level >= 2
  const passwordHasError = password.length > 0 && !passwordMeetsFloor
        ...
        <div style={{ position: 'relative' }}>
          <input
            ...
            style={{
              ...
              paddingRight: 50,
              borderRadius: tokens.radius.lg,
              border: `1px solid ${touchedFields.password && passwordHasError ? 'var(--color-accent-error)' : 'var(--glass-border-light)'}`,
              background: 'var(--color-bg-tertiary)',
              color: tokens.colors.text.primary,
              fontSize: 16,
              outline: 'none',
            ...
            aria-invalid={touchedFields.password ? passwordHasError : undefined}
          />
          <button
            type="button"
