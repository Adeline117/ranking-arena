...
interface GroupSettingsProps {
  ...
  isPro: boolean
  langTab: 'zh' | 'en'
  setLangTab: (v: 'zh' | 'en') => void
  showMultiLang: boolean
  setShowMultiLang: (v: boolean) => void
  submitting: boolean
  onSubmitEdit: () => void
  onCancelEdit: () => void
...
export default function GroupSettings({
  ...
  const addRule = () => {
    const zh = newRuleZh.trim()
    const en = newRuleEn.trim()
    if (!zh && !en) return
    setEditRules([...editRules, { zh, en }])
    setNewRuleZh('')
    setNewRuleEn('')
  }
  ...
  return (
    <Card title={t('groupSettings')}>
      ...
      <Box style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[5] }}>
        ...
        <Box>
          <Box
            style={{ display: 'flex', borderBottom: `1px solid ${tokens.colors.border.primary}` }}
          >
            <button
              type="button"
          ...
          {showMultiLang && (
            ...
            >
              ...
              <Box>
                <label style={labelStyle}>{t('groupNameEn')}</label>
                <input
                  type="text"
                  value={editNameEn}
              ...
              <Box>
                <label style={labelStyle}>{t('groupDescriptionEn')}</label>
                <textarea
        ...
        {/* Pro exclusive — upsell shown to free group owners instead of hiding the setting */}
        {editMode && !isPro && <ProGate variant="inline" featureKey="proExclusiveGroupDesc" />}
        ...
        >
          {editMode ? (
            <>
              ...
              <Button variant="primary" onClick={onSubmitEdit} disabled={submitting}>
                {submitting ? t('submitting') : t('submitChanges')}
              </Button>
            </>
          ) : (
