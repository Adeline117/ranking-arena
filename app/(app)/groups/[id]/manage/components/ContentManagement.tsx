...
export default function ContentManagement({
  ...
  return (
    <Box style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[6] }}>
      ...
      {/* 帖子 */}
      <Card
        title={`${t('posts')} (${filteredPosts.length}${contentSearch ? `/${posts.length}` : ''})`}
      >
        <Box style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[2] }}>
            ...
            <Box
              ...
              style={{
                ...
                background: post.deleted_at
                  ...
                  : post.is_pinned
                    ? `linear-gradient(135deg, ${alpha(tokens.colors.accent?.primary || tokens.colors.accent.brand, 8)} 0%, ${tokens.colors.bg.secondary} 100%)`
                    : tokens.colors.bg.secondary,
                borderRadius: tokens.radius.lg,
                border: `1px solid ${post.deleted_at ? 'var(--color-accent-error-20)' : post.is_pinned ? `${alpha(tokens.colors.accent?.primary || tokens.colors.accent.brand, 31)}` : tokens.colors.border.primary}`,
              }}
            >
              <Box
                style={{
              ...
              >
                <Box style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {post.is_pinned && (
                      <span
                        style={{
                          ...
                          background: `${alpha(tokens.colors.accent?.primary || tokens.colors.accent.brand, 13)}`,
                    ...
                    <Text
                      weight="bold"
                      style={{ textDecoration: post.deleted_at ? 'line-through' : 'none' }}
                    >
                      {post.title}
                    </Text>
                  </Box>
                  <Text size="xs" color="tertiary">
                    @{post.author_handle} ·{' '}
                    {new Date(post.created_at).toLocaleString(getLocaleFromLanguage(language))}
                  </Text>
                  {post.deleted_at && (
          ...
          {filteredPosts.length === 0 && (
            <Text color="tertiary" style={{ textAlign: 'center', padding: tokens.spacing[4] }}>
              {contentSearch ? t('noMatchingPosts') : t('noPostsYet')}
            </Text>
          )}
          {hasMorePosts && !contentSearch && (
            <Box style={{ textAlign: 'center', marginTop: tokens.spacing[3] }}>
              <Button variant="secondary" onClick={onLoadMorePosts} disabled={loadingMorePosts}>
                {loadingMorePosts ? t('loading') : t('loadMore')}
              </Button>
            </Box>
          )}
        </Box>
      ...
      {/* 评论 */}
      <Card
        title={`${t('comments')} (${filteredComments.length}${contentSearch ? `/${comments.length}` : ''})`}
      >
        <Box style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[2] }}>
            ...
            >
              ...
              >
                <Box>
                  <Text
                    size="sm"
                    style={{ textDecoration: comment.deleted_at ? 'line-through' : 'none' }}
                  >
          ...
          {filteredComments.length === 0 && (
            <Text color="tertiary" style={{ textAlign: 'center', padding: tokens.spacing[4] }}>
              {contentSearch ? t('noMatchingComments') : t('noCommentsYet')}
            </Text>
          )}
        </Box>
      </Card>
