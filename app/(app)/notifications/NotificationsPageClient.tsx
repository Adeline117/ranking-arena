...
function getNotificationBorderColor(
  type: string,
...
export default function NotificationsPageClient() {
  ...
  const { email, accessToken, authChecked: _authChecked } = useAuthSession()
  ...
  useNotificationsRealtime(
    userId,
    useCallback((newNotif: Record<string, unknown>) => {
    ...
    async (offset = 0, append = false) => {
      ...
      try {
        const res = await fetch(
          `/api/notifications?limit=${NOTIFICATION_PAGE_SIZE}&offset=${offset}`,
          {
  ...
  const handleNotificationClick = async (notification: GroupedNotification) => {
    if (!notification.read && accessToken) {
      ...
      const idsToMark =
        notification.isGrouped && notification.groupedIds.length > 1
  ...
  const effectiveFilterType =
    !features.social && SOCIAL_NOTIFICATION_TYPES.includes(filterType) ? 'all' : filterType
          ...
          {loading ? (
            ...
            >
              {groupedFiltered.map((n) => {
                const severity =
                  n.type === 'trader_alert' ? getSeverityFromMessage(n.message) : 'info'

                // Build digest title for grouped notifications
                const displayTitle = n.isGrouped ? formatGroupedTitle(n, t, language) : n.title
                    ...
                    {n.isGrouped && n.actors.length > 1 ? (
                      ...
                      >
                        {n.type === 'trader_alert' ? (
                          ...
                          <NotificationIconSvg type={n.type} size={16} />
                        )}
                      </Box>
                    )}
