...
이를 통해 리더보드는 정직하게 유지됩니다: 일관되고 자본이 충분하며 리스크를 인식하는 트레이더가 상위로 올라갑니다.
  ...
  },
  {
    ...
    excerpt: {
      en: 'How Arena aggregates rankings from 18+ active exchange sources into a unified leaderboard.',
      zh: 'Arena 如何将 18+ 个活跃交易所数据源的排名聚合为一个统一排行榜。',
...
Arena aggregates trader performance data from **18+ active exchange sources** — both centralized (CEX) and decentralized (DEX) — into a single unified leaderboard.
...
1. **Fetch**: Collect raw data from all active exchange sources
...
Arena 将来自 **18+ 个活跃交易所数据源**——包括中心化（CEX）与去中心化（DEX）——的交易员表现数据聚合为一个统一排行榜。
...
Arena tracks **18+ active exchange sources** spanning centralized (CEX) and decentralized (DEX) platforms. Each source has its own leaderboard, ranked by Arena Score.
...
Arena 追踪 **18+ 个活跃交易所数据源**，横跨中心化（CEX）与去中心化（DEX）平台。每个数据源都有自己的排行榜，按 Arena 评分排名。
