...
이를 통해 리더보드는 정직하게 유지됩니다: 일관되고 자본이 충분하며 리스크를 인식하는 트레이더가 상위로 올라갑니다.
  ...
  },
  {
    ...
    excerpt: {
      en: `How Arena aggregates rankings from ${PRODUCT_FACTS.fallbackExchangeCount}+ active source families into a unified leaderboard.`,
      zh: `Arena 如何将 ${PRODUCT_FACTS.fallbackExchangeCount}+ 个活跃来源家族的排名聚合为一个统一排行榜。`,
      ja: `Arena が ${PRODUCT_FACTS.fallbackExchangeCount} 以上のアクティブなソースファミリーを 1 つの統合リーダーボードにまとめる仕組み。`,
      ko: `Arena가 ${PRODUCT_FACTS.fallbackExchangeCount}개 이상의 활성 소스 패밀리를 하나의 통합 리더보드로 집계하는 방법.`,
    },
    content: {
      en: `
# Understanding Crypto Trader Rankings

Arena aggregates trader performance data from **${PRODUCT_FACTS.fallbackExchangeCount}+ active source families** — both centralized (CEX) and decentralized (DEX) — into a unified leaderboard.
...
Arena 将来自 **${PRODUCT_FACTS.fallbackExchangeCount}+ 个活跃来源家族**——包括中心化（CEX）与去中心化（DEX）——的交易员表现数据聚合为一个统一排行榜。
...
Arena は中央集権型（CEX）と分散型（DEX）にまたがる **${PRODUCT_FACTS.fallbackExchangeCount} 以上のアクティブなソースファミリー**のパフォーマンスデータを、統合リーダーボードにまとめます。
...
Arena는 중앙화(CEX)와 탈중앙화(DEX)를 아우르는 **${PRODUCT_FACTS.fallbackExchangeCount}개 이상의 활성 소스 패밀리** 성과 데이터를 통합 리더보드로 집계합니다.
...
Arena에는 내장 소셜 레이어가 있습니다. 그룹에 참여하고, 매매 아이디어를 올리고, 다른 트레이더와 전략을 논의해 보세요.
  ...
  },
  {
    ...
    excerpt: {
      en: `A methodology-aware view across Binance, Bybit, Hyperliquid, and ${PRODUCT_FACTS.fallbackExchangeCount}+ active source families.`,
      zh: `对 Binance、Bybit、Hyperliquid 及 ${PRODUCT_FACTS.fallbackExchangeCount}+ 个活跃来源家族的方法感知分析。`,
      ja: `Binance、Bybit、Hyperliquid と ${PRODUCT_FACTS.fallbackExchangeCount} 以上のアクティブなソースファミリーを方法論込みで比較します。`,
      ko: `Binance, Bybit, Hyperliquid 및 ${PRODUCT_FACTS.fallbackExchangeCount}개 이상의 활성 소스 패밀리를 방법론에 맞춰 비교합니다.`,
    },
    content: {
      en: `
# Top Traders by Exchange

Arena tracks **${PRODUCT_FACTS.fallbackExchangeCount}+ active source families** spanning centralized (CEX) and decentralized (DEX) platforms. Rankings only compare metrics that pass source-specific methodology and quality gates.
...
Arena 追踪横跨中心化（CEX）与去中心化（DEX）的 **${PRODUCT_FACTS.fallbackExchangeCount}+ 个活跃来源家族**。只有通过来源方法与质量门的指标才会进入对比。
...
Arena は中央集権型（CEX）と分散型（DEX）にまたがる **${PRODUCT_FACTS.fallbackExchangeCount} 以上のアクティブなソースファミリー**を追跡しています。ソース固有の方法論と品質ゲートを通過した指標だけを比較します。
...
Arena는 중앙화(CEX)와 탈중앙화(DEX)를 아우르는 **${PRODUCT_FACTS.fallbackExchangeCount}개 이상의 활성 소스 패밀리**를 추적합니다. 소스별 방법론과 품질 게이트를 통과한 지표만 비교합니다.
