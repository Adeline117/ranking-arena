...
export default function DataHealthPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/monitoring/freshness')
      ...
      .then((d) => {
        ...
        const platforms: PlatformHealth[] = (d.platforms || []).map(
          (p: Record<string, unknown>) => ({
            ...
            fieldCoverage: p.fieldCoverage as
              | { roi: number; winRate: number; maxDrawdown: number }
              | undefined,
          })
        )
        setData({
  ...
  if (loading)
    return <div style={{ padding: 40, color: 'var(--color-text-secondary)' }}>{t('loading')}</div>
  if (error)
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        <p style={{ color: 'var(--color-accent-error)', marginBottom: 12 }}>
          {t('error')}: {error}
        </p>
        <button
          onClick={retryFetch}
              ...
              .map((p) => (
                ...
                >
                  ...
                  >
                    {p.age_hours != null
                      ? p.age_hours < 1
                        ? t('dataHealthMinutes').replace(
                            '{n}',
                            String(Math.round(p.age_hours * 60))
                          )
                        : p.age_hours < 24
                          ? t('dataHealthHours').replace('{n}', String(Math.round(p.age_hours)))
                  ...
                  <td
                    style={{
                      ...
                      fontSize: 12,
                      color:
                        (p.fieldCoverage?.maxDrawdown ?? 0) >= 50
                          ? 'var(--color-accent-success)'
                          : tokens.colors.text.tertiary,
                    }}
                  >
