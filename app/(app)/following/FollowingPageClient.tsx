...
export default function FollowingPageClient() {
  ...
  const availablePlatforms = useMemo(() => {
    ...
    visibleItems.forEach((i) => {
      if (i.source) platforms.add(i.source)
    })
  ...
  const sortedItems = useMemo(() => {
    ...
    if (platformFilter !== 'all' && !(platformFilter === 'user' && !features.social)) {
      filtered = filtered.filter(
        (i) => i.source === platformFilter || (platformFilter === 'user' && i.type === 'user')
      )
    }
            ...
            <>
              ...
              >
                ...
                <StatCard
                  ...
                  color={
                    stats.avgRoi >= 0 ? tokens.colors.accent.success : tokens.colors.accent.error
                  }
                />
                <StatCard
              ...
              >
                ...
                >
                  ...
                  {features.social && <option value="user">{t('users')}</option>}
                  {availablePlatforms.map((p) => (
                    <option key={p} value={p}>
                      {getSourceDisplayName(p, language)}
                    </option>
                  ))}
                </select>
              </Box>
