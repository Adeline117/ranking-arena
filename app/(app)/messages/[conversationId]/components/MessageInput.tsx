        ...
        try {
          ...
          if (res.ok) {
            setPendingAttachment({
              ...
              type: data.category,
              fileName: data.fileName,
              originalName: data.originalName || 'pasted-image.png',
              fileSize: data.fileSize,
            })
          } else {
            showToast(data.error || t('uploadFailed'), 'error')
    ...
    >
        ...
        >
          {pendingAttachment.type === 'image' ? (
            <Image
              ...
              onClick={() => onPreviewOpen({ type: 'image', url: pendingAttachment.url })}
              unoptimized
            />
          ) : pendingAttachment.type === 'video' ? (
            <Box
              onClick={() => onPreviewOpen({ type: 'video', url: pendingAttachment.url })}
              style={{
                width: 60,
                height: 60,
                borderRadius: tokens.radius.md,
          ...
          <Box style={{ flex: 1, minWidth: 0 }}>
            ...
            <Text size="xs" color="tertiary">
              {pendingAttachment.fileSize ? formatFileSize(pendingAttachment.fileSize) : ''} •{' '}
              {getMediaTypeLabel(pendingAttachment.type, t)}
            </Text>
          </Box>
          <button
      ...
      <Box
        style={{
          ...
          padding: `${tokens.spacing[2]} ${tokens.spacing[2]} ${tokens.spacing[2]} ${tokens.spacing[3]}`,
      ...
      >
        ...
        <button
          onClick={onSend}
          disabled={
            (!newMessage.trim() && !pendingAttachment) || sending || newMessage.length > 2000
          }
          style={{
            width: 42,
            height: 42,
            ...
            border: 'none',
            background:
              (newMessage.trim() || pendingAttachment) && newMessage.length <= 2000
