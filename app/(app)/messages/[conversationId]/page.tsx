...
export default function ConversationPage({
  ...
  const otherIsTyping =
    msgHook.otherUser && conversationId ? isUserTyping(msgHook.otherUser.id, conversationId) : false
        ...
        >
          ...
          >
            ...
            <Text
              size="lg"
              weight="bold"
              style={{ marginBottom: tokens.spacing[2], color: tokens.colors.text.primary }}
            >
              {t('pleaseLogin')}
            </Text>
            <Text
  ...
  const visibleMessages = clearedBefore
    ? msgHook.messages.filter((msg) => new Date(msg.created_at) > new Date(clearedBefore))
    : msgHook.messages
  ...
  return (
    ...
    >
      {fileHook.isDragging && (
        ...
        >
          <Text size="lg" weight="bold" style={{ color: tokens.colors.accent.brand }}>
            {t('dragDropHint')}
          </Text>
        </Box>
      )}
      ...
      >
        {msgHook.hasMore && (
          <Box style={{ textAlign: 'center', marginBottom: 12 }}>
            <button
              onClick={msgHook.loadOlderMessages}
              disabled={msgHook.loadingMore}
              style={{
                padding: '6px 16px',
                background: tokens.colors.bg.secondary,
                border: `1px solid ${tokens.colors.border.primary}`,
                ...
                fontSize: 13,
                cursor: msgHook.loadingMore ? 'not-allowed' : 'pointer',
                opacity: msgHook.loadingMore ? 0.6 : 1,
        ...
        {messageGroups.map((group, groupIndex) => (
          <Box key={groupIndex}>
            ...
            {group.messages.map((msg: Message, msgIndex: number) => {
              ...
              const nextMsg =
                msgIndex < group.messages.length - 1 ? group.messages[msgIndex + 1] : null
              ...
              return (
                <MessageBubble
                  ...
                  t={t}
                  messageRef={(el) => {
                    msgHook.messageRefs.current[msg.id] = el
                  }}
                />
              )
            })}
          ...
          >
            ...
            <Box>
              <Text
                size="base"
                weight="bold"
                style={{ marginBottom: 4, color: tokens.colors.text.primary }}
              >
                {t('startConversation')}
              </Text>
              <Text size="sm" color="tertiary">
                {t('sendFirstMessage').replace(
                  '{handle}',
      ...
      {conversationId && accessToken && (
        <ChatSearchOverlay
          ...
          onNavigateToMessage={(messageId) => {
            setSearchOpen(false)
            setTimeout(() => msgHook.navigateToMessage(messageId), 100)
          }}
        />
      )}
      {msgHook.otherUser && conversationId && accessToken && (
        <ChatSettingsDrawer
          ...
          conversationId={conversationId}
          otherUser={{
            id: msgHook.otherUser.id,
            handle: msgHook.otherUser.handle || null,
            avatar_url: msgHook.otherUser.avatar_url ?? undefined,
            bio: msgHook.otherUser.bio ?? undefined,
          }}
          accessToken={accessToken}
          onSettingsChange={(newSettings) => {
            setRemark(newSettings.remark)
            if (newSettings.cleared_before) setClearedBefore(newSettings.cleared_before)
          }}
          onSearchOpen={() => setSearchOpen(true)}
          onClearHistory={() => {
      ...
      {previewOpen && (
        <MediaPreview preview={previewOpen} onClose={() => setPreviewOpen(null)} t={t} />
      )}
      ...
      <MessageInput
        ...
        setNewMessage={handleTypingChange}
        pendingAttachment={fileHook.pendingAttachment}
        setPendingAttachment={fileHook.setPendingAttachment}
        sending={sending}
        uploading={fileHook.uploading}
        setUploading={fileHook.setUploading}
        userId={userId}
        conversationId={conversationId}
        showStickerPicker={showStickerPicker}
        setShowStickerPicker={setShowStickerPicker}
        onSend={handleSend}
        onVoiceSent={msgHook.handleVoiceSent}
        onPreviewOpen={setPreviewOpen}
        showToast={showToast}
        t={t}
        language={language}
