    ...
    onInsert: (newMsg) => {
      if (newMsg.receiver_id === userId) {
        ...
        if (accessToken && conversationId) {
          fetch('/api/messages/read', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${accessToken}`, ...getCsrfHeaders() }, body: JSON.stringify({ conversationId }) }).catch(err => console.warn('[useConversationMessages] mark-read failed', err))
        }
      }
    },
