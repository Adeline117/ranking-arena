...
export default function SpotMarket({
  ...
  spotData?: SharedSpotCoin[]
  onTokenClick?: (token: SpotCoin) => void
  sectorFilter?: string | null
  initialData?: SpotCoin[]
} = {}) {
  const { t } = useLanguage()
  ...
  useEffect(() => {
    // The sparklines payload is ~152KB (largest market-data fetch, bigger than the
    // 100-coin spot data itself) and feeds the below-the-fold 7D column — not first
    // paint. Defer it to idle so it doesn't compete with hydration/interaction.
    let cancelled = false
    const load = () => {
      if (cancelled) return
  ...
  const merged = useMemo(
    () =>
      data.map((coin) => {
        ...
        if (!rt) return coin
        return { ...coin, price: rt.price ?? coin.price, change24h: rt.change24h ?? coin.change24h }
      }),
    [data, realtimePrices]
  )
  ...
  const columns: Column<SpotCoin>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (r) => (
          <span
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, overflow: 'hidden' }}
          >
            {r.image ? (
              <Image
                src={r.image}
      ...
      },
      {
        ...
        sortable: true,
        render: (r) => (
          <FlashPrice value={formatPrice(r.price)} flash={flashes[r.symbol.toUpperCase()]} />
        ),
      },
      {
        key: 'change24h',
