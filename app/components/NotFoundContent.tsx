...
// Shared 404 content, rendered by BOTH:
//   - app/not-found.tsx        (root — completely unmatched URLs, NO TopNav in tree)
//   - app/(app)/not-found.tsx  (segment — notFound() thrown inside (app) pages,
//                               which DO render inside the TopNav app shell)
//
// `inAppShell` controls the difference: the root version renders its own mini
// fixed header (there is no other navigation on a bare 404), while the (app)
// version must NOT — a second fixed header at zIndex.sticky (200) stacked on
// top of TopNav intercepted clicks on the EN/theme/Login buttons (dead-click
// bug, QA exhaustive-sweep 2026-07).
...
export default function NotFoundContent({ inAppShell = false }: { inAppShell?: boolean }) {
  ...
  // TopNav (app shell) is sticky and occupies 56px of document flow — subtract
  // it so the centered content doesn't force extra scroll.
  const minHeight = inAppShell ? 'calc(100vh - 56px)' : '100vh'
  ...
  if (!mounted) {
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: 18, marginBottom: 24, color: tokens.colors.text.secondary }}>
          Page not found
        </p>
        <a
          href="/"
  ...
  return (
    <>
      {/* Mini header for navigation — ONLY on the bare root 404 (no TopNav there).
          Inside the (app) shell TopNav already provides navigation, and a second
          fixed header would sit on top of it and swallow its clicks. */}
      {!inAppShell && (
        <header
          style={{
            position: 'fixed',
        ...
        >
          ...
          >
            <span style={{ fontSize: 24, fontWeight: 800, color: tokens.colors.accent.brand }}>
              Arena
            </span>
          </Link>
          <Link
      ...
      <div
        style={{
          ...
          justifyContent: 'center',
          padding: 24,
          // Root version: account for its own fixed 60px mini header.
          // (app) version: TopNav is sticky (in flow) — no offset needed.
          paddingTop: inAppShell ? 24 : 84,
