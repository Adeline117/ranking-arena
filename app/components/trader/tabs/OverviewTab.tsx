...
const AdvancedMetricsCard = dynamic(() => import('@/app/components/trader/AdvancedMetricsCard'), {
  ssr: false,
})
const DailyReturnsChart = dynamic(
  () =>
    import('@/app/components/trader/charts/DailyReturnsChart').then((m) => ({
      default: m.DailyReturnsChart,
    })),
  {
    ssr: false,
...
const DrawdownChart = dynamic(
  () =>
    import('@/app/components/trader/charts/DrawdownChart').then((m) => ({
...
const CopyTradeSimulator = dynamic(() => import('@/app/components/trader/CopyTradeSimulator'), {
  ssr: false,
})
const ClaimTraderButton = dynamic(() => import('@/app/components/trader/ClaimTraderButton'), {
  ssr: false,
})
const VerifiedTraderEditor = dynamic(() => import('@/app/components/trader/VerifiedTraderEditor'), {
  ssr: false,
})
...
export interface OverviewTabProps {
  ...
  positionSummary:
    | { avgLeverage: number | null; longPositions: number | null; shortPositions: number | null }
    | null
    | undefined
  ...
  return (
    ...
    >
      <Box
        ...
        style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[6], minWidth: 0 }}
      >
        ...
        {(() => {
          const curve =
            traderEquityCurve?.[selectedPeriod as EquityCurvePeriod] ?? traderEquityCurve?.['90D']
        ...
        {(() => {
          ...
          return (
            <SectionErrorBoundary>
              <CopyTradeSimulator equityCurve={simCurve} />
            </SectionErrorBoundary>
          )
        })()}
        ...
        {(() => {
          ...
          const dailyReturns = curve
            .slice(1)
            .map((point: { date: string; roi: number }, i: number) => ({
              date: point.date,
              returnPct:
                Math.abs(curve[i].roi ?? 0) > 0.001
        ...
        {(() => {
          ...
          const avgHolding =
            (perf?.avg_holding_time_hours as number | null) ?? data.avg_holding_hours ?? null
        ...
        {/* Trading Style Radar */}
        {(data.profitability_score != null ||
          data.risk_control_score != null ||
          data.execution_score != null) && (
          <Box
            className="glass-card"
            style={{
              padding: tokens.spacing[5],
