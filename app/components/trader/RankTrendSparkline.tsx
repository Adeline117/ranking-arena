  ...
  return (
    ...
    >
      <svg
        ...
        role="img"
        aria-label={`Arena score trend over 30D: ${first.toFixed(1)} to ${last.toFixed(1)}, ${isUp ? 'up' : 'down'} ${Math.abs(last - first).toFixed(1)}`}
        ...
        {(() => {
          ...
          return (
            <circle
              ...
              r={2.5}
              fill={strokeColor}
              stroke="var(--color-bg-primary, #0f0f23)"
              strokeWidth={1}
            />
          )
        })()}
        {tooltip && (
          <>
            ...
            <circle
              ...
              cy={
                padding +
                (1 - ((tooltip.point.arenaScore as number) - minScore) / range) *
                  (height - padding * 2)
              }
              r={3}
              fill={strokeColor}
