...
export function DailyReturnsChart({ data }: DailyReturnsChartProps) {
  ...
  // Use sentiment colors from design tokens with opacity variants for gradient
  const bear = tokens.colors.sentiment.bear
  const bull = tokens.colors.sentiment.bull
  const bucketDefs: Omit<Bucket, 'count'>[] = [
    { label: '<-5%', min: -Infinity, max: -5, color: bear },
    { label: '-5~-2%', min: -5, max: -2, color: `color-mix(in srgb, ${bear} 70%, transparent)` },
    { label: '-2~0%', min: -2, max: 0, color: `color-mix(in srgb, ${bear} 45%, transparent)` },
    { label: '0~2%', min: 0, max: 2, color: `color-mix(in srgb, ${bull} 45%, transparent)` },
    { label: '2~5%', min: 2, max: 5, color: `color-mix(in srgb, ${bull} 70%, transparent)` },
    { label: '>5%', min: 5, max: Infinity, color: bull },
  ]
  ...
  function valueToX(value: number): number {
    ...
    for (let i = 0; i < buckets.length; i++) {
      if (value >= buckets[i].min && value < buckets[i].max) {
        ...
        const bucketRange =
          buckets[i].max === Infinity
            ? 10
            : buckets[i].min === -Infinity
              ? 10
              : buckets[i].max - buckets[i].min
