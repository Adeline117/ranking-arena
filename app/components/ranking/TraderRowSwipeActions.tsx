...
export function TraderRowSwipeActions({
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      // Direction lock: if vertical movement dominates, bail and let page scroll
      if (!swipeRef.current.swiping) {
        if (Math.abs(dy) > 10) return // vertical wins — stop tracking entirely
        if (Math.abs(dx) <= 10) return // dead zone — wait for clear direction
        // Horizontal movement > 10px AND dominates vertical — start swiping
  ...
  return (
    ...
    >
      <div className="swipe-row-actions">
        ...
        <button onClick={handleShare} style={SWIPE_SHARE_BTN_STYLE} title={i18nTFn('share')}>
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
