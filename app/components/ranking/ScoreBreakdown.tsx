...
function ScoreBar({ label, score, maxScore, color }: BarProps) {
  ...
  return (
    <Box style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
      <Text
        size="xs"
        style={{ width: 56, flexShrink: 0, color: tokens.colors.text.secondary, fontSize: 11 }}
      >
        {label}
      </Text>
      <Box
      ...
      >
        <Box
          style={{
            width: `${pct}%`,
            height: '100%',
            background: `linear-gradient(90deg, ${color}, ${alpha(color, 87)})`,
            borderRadius: tokens.radius.md,
            transition: 'width 0.6s ease',
            minWidth: score != null ? 4 : 0,
            boxShadow: `0 0 6px ${alpha(color, 25)}`,
          }}
        />
        <Text
          size="xs"
...
export const ScoreBreakdown = memo(function ScoreBreakdown(props: ScoreBreakdownProps) {
  ...
  const hasAnyScore =
    profitability_score != null || risk_control_score != null || execution_score != null

  const explanation = hasAnyScore
    ? generateExplanation({
        profitability_score,
        risk_control_score,
        execution_score,
    ...
    : arena_score != null
      ? t('scoreBasisRoiPnl') ||
        'Score based on ROI and PnL data. Detailed breakdown not yet available for this trader.'
      : t('scoreNoDetails') ||
    ...
    >
      ...
      <Box style={{ flex: '1 1 200px', minWidth: 0, opacity: hasAnyScore ? 1 : 0.5 }}>
        <Text
          size="xs"
          weight="bold"
          style={{ marginBottom: 8, color: tokens.colors.text.secondary }}
        >
          ...
          <Box style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span
              style={{
                ...
                background: alpha(completenessColor, 9),
                border: `1px solid ${alpha(completenessColor, 25)}`,
              }}
            >
              {completenessLabel}
            </span>
          ...
          {(score_completeness === 'partial' || score_completeness === 'minimal') && (
            <Text size="xs" style={{ color: completenessColor, opacity: 0.85, lineHeight: 1.5 }}>
              {score_completeness === 'minimal'
                ? t('confidenceMinimalReason') ||
                  `Score based on limited data: ${[!win_rate && 'win rate', !max_drawdown && 'drawdown'].filter(Boolean).join(' and ')} not available`
                : t('confidencePartialReason') ||
                  `Score adjusted: ${[!win_rate && 'win rate', !max_drawdown && 'drawdown'].filter(Boolean).join(' and ')} not available from this exchange`}
            </Text>
          )}
        </Box>
      </Box>
