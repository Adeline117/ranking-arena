...
function buildShareText(data: ShareData, t: (key: string) => string): string {
  switch (data.type) {
    ...
    case 'post': {
      ...
      return t('sharePostText').replace('{title}', title).replace('{url}', data.url)
    }
    default:
      return data.url
  }
...
function DropdownItem({
  ...
  return (
    <button
      ...
      onFocus={(e) => {
        e.currentTarget.style.background = tokens.colors.bg.tertiary
        e.currentTarget.style.outline = `2px solid ${tokens.colors.accent.brand}`
    ...
    >
      <span style={{ color: tokens.colors.text.tertiary, display: 'flex', alignItems: 'center' }}>
        {icon}
      </span>
      {label}
    </button>
