...
function buildPostQueryParams(opts: {
  pageSize: number
  offset: number
  sortBy?: string
  sortType: SortType
...
export default function PostFeed(props: PostFeedProps = {}): React.ReactNode {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !loadingMore && !loading) loadMorePosts()
      },
  ...
  if (error)
    return (
      ...
      >
        ...
        <div
          style={{ color: tokens.colors.text.tertiary, fontSize: tokens.typography.fontSize.xs }}
        >
          {t('networkError')}
        </div>
        <button
    ...
    return (
      <div>
        {props.showSortButtons && (
          <SortButtons sortType={sortType} setSortType={setSortType} t={t} />
        )}
        <div
          style={{
        ...
        >
          ...
          >
            <path
              d="M30 6C30 6 32 8 32 12C32 18 26 22 20 26L14 30L16 24C20 18 24 12 30 6Z"
              stroke="currentColor"
              strokeWidth="1.5"
  ...
  return (
    <SectionErrorBoundary>
      {props.showRefreshButton && (
        <div
          style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: tokens.spacing[2] }}
        >
          <button
            ...
            onMouseEnter={(e) => {
              if (!refreshing) e.currentTarget.style.background = tokens.colors.bg.tertiary
            }}
            onMouseLeave={(e) => {
      ...
      {props.layout === 'masonry' && (
        <div
          ...
          style={{ display: 'none', justifyContent: 'flex-end', marginBottom: 8 }}
        >
          <button
            onClick={() => setMobileViewMode((prev) => (prev === 'list' ? 'masonry' : 'list'))}
            aria-label={mobileViewMode === 'list' ? t('postSwitchToGrid') : t('postSwitchToList')}
            style={{
              padding: '10px 14px',
              borderRadius: tokens.radius.md,
              border: `1px solid ${tokens.colors.border.primary}`,
      ...
      <div
        style={props.layout === 'masonry' ? { columnGap: 12 } : undefined}
        className={`stagger-children${props.layout === 'masonry' ? ' post-feed-masonry' : ''} ${props.layout === 'masonry' ? `mobile-view-${mobileViewMode}` : ''}`}
      >
