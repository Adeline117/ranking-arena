    ...
    <SectionErrorBoundary>
      ...
      {props.layout === 'masonry' && (
        <div className="mobile-only" style={{ display: 'none', justifyContent: 'flex-end', marginBottom: 8 }}>
          <button onClick={() => setMobileViewMode(prev => prev === 'list' ? 'masonry' : 'list')} aria-label={mobileViewMode === 'list' ? (language === 'zh' ? '切换为瀑布流' : 'Switch to grid') : (language === 'zh' ? '切换为列表' : 'Switch to list')} style={{ padding: '6px 12px', borderRadius: tokens.radius.md, border: `1px solid ${tokens.colors.border.primary}`, background: 'transparent', color: tokens.colors.text.secondary, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
