...
export default function PortfolioAnalytics({ positions, snapshots }: PortfolioAnalyticsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const worstPosition = positions.reduce(
      (worst, p) => (p.pnl < worst.pnl ? p : worst),
      positions[0]
    )
  ...
  const pnlColor = (v: number) =>
    v >= 0 ? tokens.colors.accent.success : tokens.colors.accent.error
...
function StatCard({
  ...
  return (
    ...
    >
      <span
        style={{ fontSize: tokens.typography.fontSize.xs, color: tokens.colors.text.secondary }}
      >
        {label}
      </span>
      <span
      ...
      {sublabel && (
        // eslint-disable-next-line no-restricted-syntax -- off-scale by design (11px sublabel keeps hierarchy below 12px label)
        <span style={{ fontSize: 11, color: tokens.colors.text.tertiary }}>{sublabel}</span>
      )}
    </div>
  )
}
