...
async function ensureSystemUser(supabase: AnySupabase) {
  ...
  const { error } = await supabase
    .from('user_profiles')
    // user_profiles 无 display_name 列（用 handle 作展示名）——写它会 PGRST204 500
    .upsert(
      {
        id: SYSTEM_USER_ID,
...
function generateMarketPost(
  ...
  if (btc || eth) {
    ...
    if (btc) {
      ...
      priceLines.push(
        `${emoji} BTC: ${formatPrice(btc.close_price)} (${formatChange(btc.daily_return_pct)})`
      )
    }
    if (eth) {
      const emoji = eth.daily_return_pct >= 0 ? '🟢' : '🔴'
      priceLines.push(
        `${emoji} ETH: ${formatPrice(eth.close_price)} (${formatChange(eth.daily_return_pct)})`
      )
    }
