...
export async function GET(request: Request) {
  ...
  try {
    ...
    // - timeout: platform unreachable or WAF-blocked with timeout (e.g., pionex CF 403 → abort)
    // - server_error: 5xx upstream error — transient, fetchers retry automatically
    const SKIP_REASONS = new Set(['auth_required', 'endpoint_gone', 'geo_blocked', 'waf_blocked', 'empty_data', 'timeout', 'server_error'])
