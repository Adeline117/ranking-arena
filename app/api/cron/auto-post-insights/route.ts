...
async function ensureSystemUser(supabase: AnySupabase) {
  ...
  if (!authUser?.user) {
    ...
    if (authErr && !authErr.message.includes('already')) {
      throw new Error(
        `Cannot create system auth user (required for user_activities FK): ${authErr.message}`
      )
    }
  }
...
async function generateRankChanges(
...
async function generateExchangeCompare(
  supabase: AnySupabase
): Promise<{ title: string; content: string }> {
  ...
  // Parallel fetch — was serial N+1 (4 sequential queries → 1 RTT)
  ...
  for (const { ex, data } of exchangeData) {
    if (data?.length) {
...
async function generateDataFact(
  supabase: AnySupabase
): Promise<{ title: string; content: string }> {
  // Count traders by various criteria — parallel (was 3 sequential queries)
  const [{ count: totalTraders }, { count: score80Plus }, { count: score90Plus }] =
    await Promise.all([
      supabase
        .from('leaderboard_ranks')
...
async function generateWeeklyRecap(
  ...
  // Parallel fetch — was 3 sequential queries, one fetching 1000 rows for a COUNT
  const [{ data: top1 }, { count: total }, { count: profitableCount }, { count: totalWithRoi }] =
    await Promise.all([
      supabase
        .from('leaderboard_ranks')
        .select('handle, source, source_trader_id, arena_score, roi')
  ...
  const profitPct = (((profitableCount || 0) / Math.max(totalWithRoi || 1, 1)) * 100).toFixed(0)
...
function formatPlatform(source: string): string {
  const map: Record<string, string> = {
    ...
    bybit: 'Bybit',
    okx_futures: 'OKX',
    hyperliquid: 'Hyperliquid', // bitget_futures removed 2026-03-18
    gmx: 'GMX',
    dydx: 'dYdX',
    drift: 'Drift',
    aevo: 'Aevo',
