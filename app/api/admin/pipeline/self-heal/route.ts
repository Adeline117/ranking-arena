 ...
 * - Data freshness from trader_latest
...
} from '@/lib/services/pipeline-self-heal'
  ...
  try {
    ...
    const platforms = await Promise.all(
      activePlatforms.map(async (platform) => {
        ...
        const [latestRes, consecutiveFailures, preferredRoute, routeFailures] = await Promise.all([
          // Latest snapshot (trader_latest = 1 row per platform+trader+window, no ORDER BY needed)
          supabase
            .from('trader_latest')
            .select('updated_at')
        ...
        // Recent record count from trader_latest (~45K rows total, cheap scan)
