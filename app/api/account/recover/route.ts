...
export const POST = withPublic(async ({ request }) => {
  ...
  if (scheduledDeletion && scheduledDeletion.getTime() < Date.now()) {
    return NextResponse.json(
      { success: false, error: 'Recovery period has expired. Account has been permanently deleted.' },
      { status: 410 }
    )
  }

  // 3. Temporarily unban the user so we can verify their credentials
  ...
  if (signInError) {
    // Re-ban the user since credentials were invalid
    await adminSupabase.auth.admin.updateUserById(userId, {
      ban_duration: '720h',
    })
    return NextResponse.json(
