...
const PLAT_L: Record<string, string> = {
...
function topPct(r: number, t: number) {
  if (!t || !r) return ''
  const p = r / t
  if (p <= 0.01) return 'Top 1%'
  if (p <= 0.05) return 'Top 5%'
  if (p <= 0.1) return 'Top 10%'
  if (p <= 0.25) return 'Top 25%'
  return 'Top ' + Math.ceil(p * 100) + '%'
}
...
export async function GET(request: NextRequest) {
  ...
  let q = sb
    .from('trader_sources')
    .select('handle, source, source_trader_id')
    .or(`handle.eq.${safeHandle},source_trader_id.eq.${safeHandle}`)
    .limit(1)
  ...
  const { data: lr } = await sb
    ...
    .select('rank, roi, win_rate, arena_score')
    .eq('source', tr.source)
    .eq('source_trader_id', tr.source_trader_id)
    .eq('season_id', sId)
    .maybeSingle()
  // PERF FIX: was count:exact (25s+). Read from pre-computed cache instead.
  const { data: countCache } = await sb
  ...
  const name = tr.handle || handle,
    rank = lr?.rank ?? 0,
    total = count ?? 0,
    roi = lr?.roi,
    score = lr?.arena_score,
    winRate = lr?.win_rate
  const rv = roi != null && !isNaN(roi),
    sv = score != null && !isNaN(score),
    wv = winRate != null && !isNaN(winRate)
  const rc = rv && roi >= 0 ? C.success : C.error,
    rs = rv ? fmtRoi(roi) : '--',
    tp = rank > 0 && total > 0 ? topPct(rank, total) : ''
  const pl =
    PLAT_L[tr.source] ||
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 13, color: C.dimmer, marginLeft: 4 }}>Rank Card</span>
          </div>
          <div
            style={{
              display: 'flex',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
          ...
          {pl && (
            ...
            >
              <div
                style={{ width: 6, height: 6, borderRadius: 999, background: pc, display: 'flex' }}
              />
              <span style={{ fontSize: 13, fontWeight: 700, color: pc }}>{pl}</span>
            </div>
          )}
        </div>
        <div style={{ display: 'flex', gap: 16, flex: 1, alignItems: 'stretch' }}>
          ...
          <div
            style={{
              ...
              padding: '20px 24px',
              borderRadius: 16,
              background: rv && roi >= 0 ? 'rgba(47,229,125,0.07)' : 'rgba(255,85,85,0.07)',
              border:
                rv && roi >= 0
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            {tp && (
              ...
              >
                <span style={{ fontSize: 13, fontWeight: 800, color: C.goldLight }}>{tp}</span>
              </div>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.purple }}>via Arena</span>
            <span style={{ fontSize: 13, color: C.dim }}>{cta}</span>
          </div>
        </div>
      </div>
    </div>,
