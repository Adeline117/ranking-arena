  ...
  return (
    <>
      ...
      <button
        onClick={async () => {
          ...
          if (isInAppBrowser) {
            ...
            try { await navigator.clipboard.writeText(oauthUrl) } catch { /* clipboard may not be available */ }
            onError(lang === 'zh'
              ? '请在系统浏览器(Safari/Chrome)中打开此页面登录X。链接已复制到剪贴板。'
              : 'Please open this page in your system browser (Safari/Chrome) to sign in with X. Link copied to clipboard.')
            window.open(oauthUrl, '_system')
            return
          }
          const { error: oauthError } = await supabase.auth.signInWithOAuth({
      ...
      >
        ...
        {lang === 'zh' ? '使用 X 登录' : 'Sign in with X'}
      </button>
      ...
      <button
        onClick={async () => {
          ...
          if (isInAppBrowser) {
            ...
            try { await navigator.clipboard.writeText(oauthUrl) } catch { /* clipboard may not be available */ }
            onError(lang === 'zh'
              ? '请在系统浏览器(Safari/Chrome)中打开此页面登录Discord。链接已复制到剪贴板。'
              : 'Please open this page in your system browser (Safari/Chrome) to sign in with Discord. Link copied to clipboard.')
            window.open(oauthUrl, '_system')
            return
          }
          const { error: oauthError } = await supabase.auth.signInWithOAuth({
      ...
      >
        ...
        {lang === 'zh' ? '使用 Discord 登录' : 'Sign in with Discord'}
      </button>
      ...
      {/* Bottom wallet section - rendered separately in page */}
    </>
  )
}
