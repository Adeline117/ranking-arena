...
export default function ExchangeRankingClient({ traders: initialTraders, exchange, totalCount }: { traders: TraderData[]; exchange?: string; totalCount?: number }) {
  ...
  const renderRowCells = useCallback((td: TraderData, originalRank: number) => { const name = getDisplayName(td); const roiColor = td.roi != null && td.roi >= 0 ? tokens.colors.accent.success : tokens.colors.accent.error; const wrColor = td.win_rate != null ? (td.win_rate >= 50 ? tokens.colors.accent.success : tokens.colors.accent.error) : tokens.colors.text.tertiary; return (<><div role="cell"><RankBadge rank={originalRank} /></div><div role="cell" style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}><div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: getAvatarGradient(td.trader_key) }}><TraderAvatarImg avatarUrl={td.avatar_url} traderKey={td.trader_key} name={name} size={32} /></div><span style={{ fontSize: 13, fontWeight: 600, color: tokens.colors.text.primary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>{(td.platform === 'web3_bot' || td.trader_type === 'bot' || td.is_bot) && <span style={{ padding: '0px 4px', borderRadius: 4, fontSize: 10, fontWeight: 600, flexShrink: 0, color: '#a78bfa', background: 'rgba(167,139,250,0.12)', border: '1px solid rgba(167,139,250,0.25)' }}>Bot</span>}</div><div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 700, color: roiColor }}>{formatROI(td.roi)}</div><div role="cell" style={{ display: 'flex', justifyContent: 'center' }}><Sparkline roi={td.roi ?? undefined} width={72} height={20} /></div><div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: wrColor }}>{td.win_rate != null ? `${td.win_rate.toFixed(2)}%` : NULL_DISPLAY}</div><div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: td.max_drawdown != null ? tokens.colors.accent.error + 'cc' : tokens.colors.text.tertiary }}>{td.max_drawdown != null ? `-${Math.abs(td.max_drawdown).toFixed(2)}%` : NULL_DISPLAY}</div>{optionalColumns.pnl && <div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: td.pnl != null ? (td.pnl >= 0 ? tokens.colors.accent.success : tokens.colors.accent.error) : tokens.colors.text.tertiary }}>{formatPnL(td.pnl)}</div>}{optionalColumns.followers && <div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: tokens.colors.text.secondary }}>{td.followers != null ? formatNumber(td.followers) : NULL_DISPLAY}</div>}{optionalColumns.sharpe_ratio && <div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: td.sharpe_ratio != null ? (td.sharpe_ratio >= 1 ? tokens.colors.accent.success : td.sharpe_ratio >= 0 ? tokens.colors.text.primary : tokens.colors.accent.error) : tokens.colors.text.tertiary }}>{td.sharpe_ratio != null ? td.sharpe_ratio.toFixed(2) : NULL_DISPLAY}</div>}{optionalColumns.trades_count && <div role="cell" style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: tokens.colors.text.secondary }}>{td.trades_count != null ? formatNumber(td.trades_count) : NULL_DISPLAY}</div>}<div role="cell" style={{ textAlign: 'right', display: 'flex', justifyContent: 'flex-end' }}>{td.arena_score != null ? <span style={{ width: 32, height: 32, borderRadius: '50%', border: `2px solid ${getScoreColor(td.arena_score)}`, background: `color-mix(in srgb, ${getScoreColor(td.arena_score)} 10%, transparent)`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: getScoreColor(td.arena_score) }}>{td.arena_score.toFixed(0)}</span> : <span style={{ fontSize: 13, color: tokens.colors.text.tertiary }}>{NULL_DISPLAY}</span>}</div></>) }, [optionalColumns])
