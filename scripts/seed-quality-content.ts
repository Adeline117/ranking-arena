...
const POST_TEMPLATES = [
  ...
  {
    content:
      'Opened a 5x long $ETH at $2,180 — thesis: ETH/BTC ratio bottomed on weekly, funding is negative, and the merge narrative is coming back with Pectra upgrade. Target $2,600, stop at $2,050. Risk/reward about 3:1.',
    lang: 'en',
  },
  {
    content:
      '今天的交易复盘：\n\n做多 SOL 在 $89 进场，$94 出场。逻辑是 Solana DEX 交易量连续三天创新高 + 大户在链上加仓。止损设在 $86 下方（4H 支撑位）。盈利 +5.6%，仓位 10% 资金。\n\n关键教训：进场时机还是太早了，应该等 4H 收线确认再进。',
    lang: 'zh',
  },
  {
    content:
      "Week 12 P&L recap:\n\n- Mon: +$420 (BTC long scalp)\n- Tue: -$180 (ETH short stopped out)\n- Wed: flat (no setup)\n- Thu: +$890 (SOL breakout trade)\n- Fri: +$340 (BTC range trade)\n\nNet: +$1,470 (+2.9% portfolio)\n\nBiggest lesson: Wednesday discipline — no setup = no trade. That's the hardest skill.",
    lang: 'en',
  },
  {
    content:
  ...
  {
    content:
      '刚仔细看了 Arena 的 Score 算法文档：\n\n- Return Score (60分): 用 tanh 函数压缩 ROI，避免极端值主导\n- Profit Score (40分): 基于绝对 PnL，大资金量有优势\n- 置信度乘数: 交易次数太少会被降权\n\n这个设计比单纯看 ROI 排名合理多了。有些人 ROI 10000% 但只做了 2 笔交易，风险极高。Arena Score 会把这种人降到很低。',
    lang: 'zh',
  },
  ...
  {
    content:
      'Just discovered that the #1 trader on GMX leaderboard has a 65% win rate with an avg holding time of 16 hours. Classic swing trader approach. Meanwhile most people are scalping with 35% win rate and wondering why they lose money.',
    lang: 'en',
  },
  {
    content:
      '昨晚 ETH 拉盘到 $2,247 的时候我在睡觉。\n\n醒来看到账户 +$800，比任何闹钟都好使。\n\n持仓过夜有风险，但这次逻辑是对的：ETH 在 $2,100 附近形成了明显的双底，加上 gas 费降到历史低位说明 layer2 分流了大量交易。做多的逻辑站得住。',
    lang: 'zh',
  },
  {
    content:
      "Funded $500 into a fresh account to test a new strategy:\n- Only trade BTC and ETH\n- Max 3x leverage\n- Only enter on 4H close above/below key levels\n- Target: double the account in 90 days\n\nWill track progress on Arena. Current score: 0. Let's see where this goes.",
    lang: 'en',
  },
]
...
function detectTopic(content: string): keyof typeof COMMENT_POOLS {
  ...
  if (/analysis|on.chain|funding|open interest|链上|分析|outflow|数据|volume/i.test(lower))
    return 'analysis'
  if (/strategy|advice|建议|position.siz|risk.manage|新人|lesson|tip|kelly|discipline/i.test(lower))
    return 'strategy'
  if (/arena|score|leaderboard|排行榜|排名|tanh|composite|methodology/i.test(lower))
    return 'platform'
  return 'general'
}
...
async function main() {
  ...
  if (mode === '--execute' || mode === '--cleanup') {
    ...
    // trigger. Do not recount and write the cached value from a maintenance
    // process: that can race with live comments and overwrite newer truth.
