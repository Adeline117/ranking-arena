...
/**
 * 使用 Puppeteer 获取 KuCoin 90天ROI排行榜数据
 */
async function fetchKucoin90dRoi() {
  console.log('=== KuCoin 90天ROI排行榜数据抓取 ===')
  console.log('')
  console.log('正在启动浏览器...')
  ...
  page.on('response', async (response) => {
    ...
    // 监听 KuCoin 相关的 API
    if (url.includes('kucoin.com') && (url.includes('api') || url.includes('leaderboard') || url.includes('copy-trading') || url.includes('trader'))) {
      try {
        ...
        const isSuccess = status === 200 && (data.code === '200000' || data.code === 200000 || data.success === true)
  ...
  try {
    console.log('正在访问 KuCoin Copy Trading 页面...')
    ...
    if (!capturedData || capturedData.length === 0) {
      ...
      // 尝试常见的 KuCoin API 端点
      const apiEndpoints = [
        'https://www.kucoin.com/api/v1/copy-trading/leaderboard',
        'https://api.kucoin.com/api/v1/copy-trading/leaderboard',
      ]
        ...
        try {
          ...
          for (const params of paramsList) {
            ...
            if (response.success && response.data) {
              ...
              if ((data.code === '200000' || data.code === 200000) && data.data) {
                const result = data.data
    ...
    writeFileSync(join(backupDir, `kucoin_90d_raw_${Date.now()}.json`), JSON.stringify(capturedData, null, 2))
    console.log(`原始数据已保存到: data/backup/kucoin_90d_raw_${Date.now()}.json`)
...
async function importToSupabase(normalizedData) {
  ...
  console.log(`完成！共导入 ${validData.length} 条 KuCoin 90天ROI Top 100 交易员数据`)
}
...
async function main() {
  try {
    ...
    if (jsonPath) {
      ...
      rawData = await fetchKucoin90dRoi()
    }
