...
function normalizeData(rawData) {
  ...
  return rawData.map((item) => {
    ...
    } else if (item.winRatePct != null) {
      const winRateNum = typeof item.winRatePct === 'string'
        ? parseFloat(item.winRatePct.replace(/[+%]/g, ''))
        : Number(item.winRatePct)
      if (!isNaN(winRateNum)) {
        winRate = winRateNum
      }
    }
...
/**
 * 使用 Puppeteer 获取 OKX 90天ROI排行榜数据
 */
async function fetchOkx90dRoi() {
  console.log('=== OKX 90天ROI排行榜数据抓取 ===')
  console.log('')
  console.log('正在启动浏览器...')
  ...
  page.on('response', async (response) => {
    ...
    // 监听 OKX 相关的 API
    if (url.includes('okx.com') && (url.includes('api') || url.includes('leaderboard') || url.includes('copy-trading') || url.includes('trader'))) {
      try {
        ...
        // 检查响应是否成功
        const isSuccess = status === 200 && (data.code === '0' || data.code === 0 || data.success === true)
  ...
  try {
    console.log('正在访问 OKX Copy Trading 页面...')
    await page.goto('https://www.okx.com/copy-trading', {
    ...
    if (!capturedData || capturedData.length === 0) {
      ...
      // 尝试常见的 OKX API 端点
      const apiEndpoints = [
        'https://www.okx.com/api/v5/copy-trading/public/leaderboard',
        'https://www.okx.com/api/v5/copy-trading/leaderboard',
        'https://www.okx.com/api/v5/tradingBot/public/leaderboard',
      ]
    ...
    writeFileSync(join(backupDir, `okx_90d_raw_${Date.now()}.json`), JSON.stringify(capturedData, null, 2))
    console.log(`原始数据已保存到: data/backup/okx_90d_raw_${Date.now()}.json`)
...
async function importToSupabase(normalizedData) {
  ...
  console.log(`完成！共导入 ${validData.length} 条 OKX 90天ROI Top 100 交易员数据`)
}
