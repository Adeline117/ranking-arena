 ...
 * 获取每个交易员的详细数据
 * 
 * 用法: node scripts/fetch_kucoin_trader_details.mjs [--limit=N]
 */
 ...
 * 获取所有 KuCoin 交易员
 */
async function getKucoinTraders(limit) {
...
async function fetchTraderDetail(browser, traderId, handle) {
  ...
  try {
    ...
    page.on('response', async response => {
      ...
      // 交易员详情 API
      if (url.includes('lead-config/detail') || url.includes('trader/detail')) {
        try {
          const data = JSON.parse(await response.text())
          if (data.data) {
            const d = data.data
            details.stats.roi = parseFloat(d.totalPnlRatio || d.thirtyDayPnlRatio || 0) * 100
            details.stats.pnl = parseFloat(d.totalPnl || 0)
            details.stats.winRate = parseFloat(d.winRate || 0) * 100
            details.stats.maxDrawdown = parseFloat(d.maxDrawdown || d.mdd || 0) * 100
            details.stats.followers = parseInt(d.followerCount || d.copierCount || 0)
            details.stats.totalTrades = parseInt(d.totalTrades || d.tradesCount || 0)
            details.stats.aum = parseFloat(d.aum || 0)
            details.stats.sharpeRatio = parseFloat(d.sharpeRatio || 0)
            details.stats.copierPnl = parseFloat(d.copierPnl || 0)
          }
        } catch {}
      }

      // 持仓历史 API
      if (url.includes('position/history') || url.includes('trade/history')) {
        try {
          const data = JSON.parse(await response.text())
          ...
          list.forEach(p => {
            details.positions.push({
              ...
              direction: (p.side || '').toLowerCase().includes('sell') ? 'short' : 'long',
              pnlPct: parseFloat(p.pnlRate || p.roi || 0) * 100,
    ...
    // 访问交易员详情页
    const url = `https://www.kucoin.com/copytrading/trader/${traderId}`
    ...
    await page.evaluate(() => {
      document.querySelectorAll('button, [class*="close"]').forEach(el => {
        ...
        if (text.includes('ok') || text.includes('got it') || text.includes('close')) {
          try { el.click() } catch {}
        }
      })
    })
    ...
    if (Object.keys(details.stats).length === 0) {
      const pageData = await page.evaluate(() => {
        ...
        // ROI
        const roiMatch = text.match(/(?:ROI|收益率|PnL Ratio)[:\s]*([+-]?\d+(?:,\d+)*\.?\d*)%/i)
        if (roiMatch) result.roi = parseFloat(roiMatch[1].replace(/,/g, ''))

        // PnL
        const pnlMatch = text.match(/(?:Total PnL|总收益|Profit)[:\s]*\$?([+-]?[\d,]+\.?\d*)/i)
        ...
        // 交易天数
        const daysMatch = text.match(/(?:Days as Lead|交易天数|Days)[:\s]*(\d+)/i)
        if (daysMatch) result.daysAsLeader = parseInt(daysMatch[1])
...
async function main() {
  ...
  // 获取交易员列表
  const traders = await getKucoinTraders(limit)
