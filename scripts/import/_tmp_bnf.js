// Binance Futures refresh helper
...
(async()=>{
  ...
  for(let p=1;p<=25;p++){
    const r=spawnSync("curl",["-s","-m","10","--compressed","-x","http://127.0.0.1:7890","-X","POST",
      ...
      "-d",JSON.stringify({pageNumber:p,pageSize:20,timeRange:"30D",dataType:"ROI",favoriteOnly:false})],{encoding:"utf8"});
    try{const d=JSON.parse(r.stdout);const list=d.data?.list||[];if(!list.length)break;
    for(const it of list)all.push({id:it.leadPortfolioId||"",n:it.nickname||"",a:it.avatarUrl||null,roi:it.roi!=null?parseFloat(it.roi)*100:null,pnl:it.pnl!=null?parseFloat(it.pnl):null,wr:it.winRate!=null?parseFloat(it.winRate)*100:null,dd:it.maxDrawDown!=null?parseFloat(it.maxDrawDown)*100:null})}catch{break}
  }
  const now=new Date().toISOString();let saved=0;
  for(let i=0;i<all.length;i+=50)try{await sb.from("trader_sources").upsert(all.slice(i,i+50).map(t=>({source:"binance_futures",source_trader_id:t.id,handle:t.n||t.id,avatar_url:t.a,market_type:"futures",is_active:true})),{onConflict:"source,source_trader_id"})}catch{}
  for(let i=0;i<all.length;i+=30){const{error}=await sb.from("trader_snapshots").upsert(all.slice(i,i+30).map((t,j)=>({source:"binance_futures",source_trader_id:t.id,season_id:"current_30d",rank:i+j+1,roi:t.roi,pnl:t.pnl,win_rate:t.wr,max_drawdown:t.dd,arena_score:cs(t.roi,t.dd,t.wr),captured_at:now})),{onConflict:"source,source_trader_id,season_id"});if(!error)saved+=Math.min(30,all.length-i)}
  console.log(saved)
})();
