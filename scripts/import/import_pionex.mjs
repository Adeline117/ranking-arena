 ...
 * Pionex 合约跟单排行榜数据抓取
 *
 * URL: https://www.pionex.com/copy-trade
 *
 * Pionex 有 Cloudflare 保护，需要使用 Playwright 抓取页面数据
...
const SOURCE = 'pionex'
const BASE_URL = 'https://www.pionex.com/copy-trade'
...
async function fetchLeaderboard(browser, period) {
  ...
  page.on('response', async (response) => {
    ...
    if (!url.includes('copy') && !url.includes('trader') && !url.includes('leader') && !url.includes('rank')) return
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        for (const t of list) {
          const traderId = String(t.uid || t.traderId || t.userId || t.id || '')
          if (!traderId || traderId === 'undefined' || traders.has(traderId)) continue

          let roi = parseFloat(String(t.roi || t.roiRate || t.returnRate || t.profitRate || 0))
          if (Math.abs(roi) > 0 && Math.abs(roi) < 1) roi *= 100

          traders.set(traderId, {
            traderId,
            nickname: t.nickname || t.traderName || t.nickName || t.name || `Trader_${traderId.slice(0, 8)}`,
            ...
            maxDrawdown: t.maxDrawdown != null ? parseFloat(String(t.maxDrawdown)) : null,
            followers: parseInt(String(t.followers || t.followerNum || t.followerCount || t.copyNum || 0)),
          })
        }
  ...
  try {
    ...
    const periodTexts = { '7D': ['7 Days', '7D', '7天'], '30D': ['30 Days', '30D', '30天'], '90D': ['90 Days', '90D', '90天'] }
    ...
    if (traders.size < 10) {
      ...
      const pageData = await page.evaluate(() => {
        const results = []
        const seen = new Set()
        document.querySelectorAll('[class*="trader"], [class*="card"], [class*="item"], [class*="list"]').forEach(card => {
          ...
          if (!nickname) return
          const traderId = 'pionex_' + nickname.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 20)
          if (!traderId || traderId === 'pionex_' || seen.has(traderId)) return
          seen.add(traderId)
          let winRate = null
          const wrMatch = text.match(/(?:Win|胜率)[:\s]*(\d{1,3}(?:\.\d{1,2})?)\s*%/i)
          if (wrMatch) winRate = parseFloat(wrMatch[1])
          results.push({ traderId, nickname, roi, winRate })
        })
        return results
      })
    ...
    await page.screenshot({ path: `/tmp/pionex_${period}_${Date.now()}.png`, fullPage: true }).catch(() => {})
  } finally {
    await page.close()
  }
...
async function main() {
  ...
  console.log(`\n${'='.repeat(50)}`)
  console.log(`Pionex Copy Trading 数据抓取 (Playwright)`)
  ...
  try {
    for (const period of periods) {
      ...
      if (traders.length === 0) {
        console.log(`  ⚠ ${period} 无数据 (API 可能不可用或被 Cloudflare 阻挡)`)
