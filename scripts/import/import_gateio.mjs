...
async function fetchLeaderboardData(period) {
  ...
  try {
    // 尝试 Gate.io copy trading / strategy bot API
    const urls = [
      `https://www.gate.io/api/v1/copy/leaders?page=1&limit=${TARGET_COUNT}&period=${periodVal}&sort=roi`,
      `https://www.gate.io/api/v4/copy-trading/rank?limit=${TARGET_COUNT}&period=${periodVal}`,
    ]
    ...
    for (const url of urls) {
      try {
        ...
        if (Array.isArray(list) && list.length > 0) {
          ...
          for (const t of list) {
            traders.push({
              traderId: String(t.uid || t.user_id || t.trader_id || ''),
              nickname: t.nickname || t.name || t.displayName || 'Unknown',
              roi: parseFloat(t.roi || t.profit_rate || 0) * (Math.abs(t.roi || 0) > 10 ? 1 : 100),
              pnl: parseFloat(t.pnl || t.profit || 0),
              winRate: t.winRate != null ? parseFloat(t.winRate) : (t.win_rate != null ? parseFloat(t.win_rate) : null),
              maxDrawdown: t.maxDrawdown != null ? parseFloat(t.maxDrawdown) : (t.max_drawdown != null ? parseFloat(t.max_drawdown) : null),
              followers: parseInt(t.followers || t.follower_num || 0),
              copiers: parseInt(t.copiers || t.copy_num || 0),
...
async function saveTraders(traders, period) {
  ...
  await supabase.from('trader_sources').upsert(
    topTraders.map(t => ({
      ...
      handle: t.nickname,
      profile_url: `https://www.gate.io/strategybot/trader/${t.traderId}`,
      is_active: true
    })),
