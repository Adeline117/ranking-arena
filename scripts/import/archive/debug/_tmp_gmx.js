...
(async()=>{
  ...
  for(let i=0;i<t.length;i+=30){const{error}=await sb.from("trader_snapshots").upsert(t.slice(i,i+30).map((x,j)=>({source:"gmx",source_trader_id:x.id,season_id:"30D",rank:i+j+1,roi:x.roi,pnl:x.pnl,win_rate:x.wr,arena_score:cs(x.roi,x.wr),captured_at:now})),{onConflict:"source,source_trader_id,season_id"});if(!error)s+=Math.min(30,t.length-i)}
  console.log(s)
})();
