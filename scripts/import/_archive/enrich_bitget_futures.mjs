/**
 * Bitget Futures Data Enrichment (Browser-based)
 * 
 * Fills missing win_rate for existing bitget_futures traders.
 * Uses puppeteer to visit each trader's page and extract data.
 * 
 * Usage: node scripts/import/enrich_bitget_futures.mjs [30D] [--batch=50] [--offset=0]
 */
...
async function extractTraderData(page, traderId, period) {
  ...
  const apiPromise = new Promise((resolve) => {
    ...
    page.on('response', async (res) => {
      const url = res.url()
      try {
        if ((url.includes('trader') || url.includes('copy')) && url.includes('api')) {
          const text = await res.text().catch(() => '')
          if (text.includes('winRate') || text.includes('maxDrawdown')) {
            try {
              const json = JSON.parse(text)
              const data = json.data || json
              if (data.winRate !== undefined || data.maxDrawdown !== undefined) {
  ...
  try {
    await page.goto(`https://www.bitget.com/copy-trading/trader/${traderId}/futures`, {
  ...
  return {
    winRate: details.winRate ?? details.winRatio ?? null,
    maxDrawdown: details.maxDrawdown ?? details.mdd ?? null,
  }
}
...
async function main() {
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`Bitget Futures Enrichment — ${period} (batch=${batch}, offset=${offset})`)
  console.log(`${'='.repeat(60)}`)

  // Get traders missing win_rate
  ...
  console.log(`Processing ${missing.length} traders (offset ${offset})...`)

  // Filter out traders with synthetic IDs (can't visit their pages)
  const validTraders = missing.filter(t => !t.source_trader_id.startsWith('spot_') && /^[a-f0-9]+$/.test(t.source_trader_id))
  console.log(`Valid trader IDs: ${validTraders.length}`)
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`✅ Bitget Futures ${period} enrichment done`)
