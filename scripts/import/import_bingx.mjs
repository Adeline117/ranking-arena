...
async function fetchLeaderboardData(period) {
  ...
  try {
    // 尝试多个可能的 API 端点
    const urls = [
      `https://bingx.com/api/uc/v1/public/copyTrade/traders?page=1&pageSize=${TARGET_COUNT}&period=${periodVal}&sortBy=roi&sortOrder=desc`,
      `https://bingx.com/bapi/copyTrade/v1/public/traders?page=1&pageSize=${TARGET_COUNT}&period=${periodVal}&sortBy=roi`,
    ]
    ...
    for (const url of urls) {
      try {
        ...
        if (list.length > 0) {
          ...
          for (const t of list) {
            traders.push({
              traderId: String(t.uniqueId || t.uid || t.traderId || ''),
              nickname: t.traderName || t.nickname || t.displayName || 'Unknown',
              roi: parseFloat(t.roi || t.roiRate || 0) * (t.roi > 10 ? 1 : 100), // 处理可能是小数或百分比
              ...
              followers: parseInt(t.followerNum || t.followers || 0),
              copiers: parseInt(t.copyNum || t.copiers || 0),
              avatarUrl: t.headUrl || t.avatar || null,
            })
          }
          break
        }
