#!/usr/bin/env node
/**
 * Bitget Spot Detail Enrichment (Browser-based)
 * 
 * Same approach as Bitget Futures but for spot traders.
 * Bitget uses the same API endpoints for both spot and futures traders.
 * The traderUid is the same; the URL suffix differs (/spot vs /futures).
 * 
 * Usage: node scripts/import/enrich_bitget_spot_detail.mjs [--limit=100] [--offset=0]
 */
...
async function main() {
  console.log(`\n${'='.repeat(60)}`)
  console.log(`Bitget Spot Detail Enrichment (browser)`)
  ...
  console.log(`Total: ${traders.length}, valid hex: ${valid.length}, has stats: ${hasStats.size}, to process: ${toProcess.length}`)
  ...
  try {
    ...
    for (let i = 0; i < toProcess.length; i++) {
        ...
        for (const [period, cycleTime] of Object.entries(CYCLE_MAP)) {
          ...
          if (cycleResult?.code === '00000' && cycleResult.data) {
            ...
            if (d.roiRows?.rows?.length > 0) {
              if (await upsertEquityCurve(tid, period, d.roiRows.rows) > 0) equityN++
            }
            if (d.statisticsDTO) {
              if (await upsertStats(tid, period, d.statisticsDTO) > 0) statsN++
            }
            if (d.symbolDistributeDetail?.webSymbolDistributeVOs?.length > 0) {
              if (await upsertAssetBreakdown(tid, period, d.symbolDistributeDetail.webSymbolDistributeVOs) > 0) assetsN++
            }
          }
      ...
      if ((i + 1) % 10 === 0 || i === toProcess.length - 1) {
        console.log(`  [${i + 1}/${toProcess.length}] stats=${statsN} eq=${equityN} assets=${assetsN} err=${errors}`)
      }
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`✅ Bitget Spot enrichment done`)
  console.log(`   Stats: ${statsN}, Equity: ${equityN}, Assets: ${assetsN}, Errors: ${errors}`)
  console.log(`${'='.repeat(60)}`)
}
