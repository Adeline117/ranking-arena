/**
 * Bitget Spot - Fast version (list only, no detail pages)
...
async function fetchAndSave(period) {
  ...
  try {
    ...
    const url = 'https://www.bitget.com/copy-trading/spot?rule=2&sort=0'
    console.log(`📱 访问: ${url}`)
    try {
  ...
  const sourcesData = records.map(t => ({
    ...
    profile_url: `https://www.bitget.com/copy-trading/trader/${t.traderId}`,
    is_active: true,
  }))
