...
async function fetchLeaderboardData(period) {
  ...
  try {
    // 尝试 Phemex copy trading API
    const urls = [
      `https://phemex.com/api/copy-trading/public/traders?days=${days}&limit=${TARGET_COUNT}&sort=roi`,
      `https://api.phemex.com/copy-trading/v1/public/rank?period=${days}&size=${TARGET_COUNT}`,
    ]
