...
describe('Solana program-state witness process boundary', () => {
  it('returns a fixed argument error without stdout, stack, or local path', async () => {
    const result = await run('node_modules/.bin/tsx scripts/dex-solana-program-state-witness.mts')

    expect(result).toEqual({
      exitCode: 64,
      stdout: '',
      stderr: 'program_state_witness_invalid_arguments\n',
    })
  })

  it('keeps exit 70 when stdout was closed before startup and does no network work', async () => {
    const result = await run(
      'exec 1>&-; node_modules/.bin/tsx scripts/dex-solana-program-state-witness.mts'
    )
  ...
  it('handles a closed stderr pipe without EPIPE, a stack, or an uncontrolled exit', async () => {
    const result = await run(
      '{ node_modules/.bin/tsx scripts/dex-solana-program-state-witness.mts; } 2>&1 | dd bs=1 count=0 2>/dev/null'
    )
