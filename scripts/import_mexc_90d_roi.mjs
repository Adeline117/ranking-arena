...
function normalizeData(rawData) {
  ...
  return rawData.map((item) => {
    ...
    // uid: "92241758" (交易员ID)
    // nickname: "92*****8" (昵称)
    // roi: 40.0936 (数字，已经是百分比格式)
    // pnl: 17975.516600000000418619 (数字)
    // winRate: 0.3065 (数字，小数格式，0.3065 表示 30.65%)
    // followers: 1 (数字)
    // avatar: "https://..." (头像URL)
    
    const traderId = item.uid || item.traderId || String(item.id || '')
    const handle = item.nickname || item.nickName || item.displayName || item.name || null
    
    // ROI 是数字格式，但需要乘以 100 转换为百分比
    // MEXC 官网显示的是 "+4,009.36%"，但 API 返回的是 40.0936，需要乘以 100
    let roi = 0
    if (item.roi != null) {
      if (typeof item.roi === 'string') {
        const roiNum = parseFloat(item.roi)
        if (!isNaN(roiNum)) {
          roi = roiNum * 100 // 乘以 100 转换为百分比
        }
      } else if (typeof item.roi === 'number') {
        roi = item.roi * 100 // 乘以 100 转换为百分比
      }
    }
    ...
    // const followers = item.followers != null ? Number(item.followers) : 
    ...
    // 胜率 (MEXC API 中 winRate 是小数格式，如 0.3065 表示 30.65%)
    let winRate = null
    if (item.winRate != null) {
      if (typeof item.winRate === 'string') {
        const winRateNum = parseFloat(item.winRate)
        if (!isNaN(winRateNum)) {
    ...
    // 多时间段ROI（如果API提供）
    const roi_7d = item.roi7d != null ? Number(item.roi7d) * 100 : (item.roi_7d != null ? Number(item.roi_7d) : null)
    const roi_30d = item.roi30d != null ? Number(item.roi30d) * 100 : (item.roi_30d != null ? Number(item.roi_30d) : null)
    const roi_1y = item.roi1y != null ? Number(item.roi1y) * 100 : (item.roi_1y != null ? Number(item.roi_1y) : null)
    const roi_2y = item.roi2y != null ? Number(item.roi2y) * 100 : (item.roi_2y != null ? Number(item.roi_2y) : null)
...
function extractRowsFromFile(fileData) {
  ...
  if (Array.isArray(fileData)) {
    rawData = fileData
  } else if (fileData.data?.data?.content && Array.isArray(fileData.data.data.content)) {
    // MEXC API 响应格式: { success: true, code: 0, data: { data: { content: [...] } } }
    rawData = fileData.data.data.content
  } else if (fileData.data?.content && Array.isArray(fileData.data.content)) {
    // MEXC API 响应格式: { success: true, code: 0, data: { content: [...] } }
...
/**
 * 使用 Puppeteer 获取 MEXC 90天ROI排行榜数据
 */
async function fetchMexc90dRoi() {
  console.log('=== MEXC 90天ROI排行榜数据抓取 ===')
  console.log('')
  console.log('正在启动浏览器...')
  ...
  page.on('response', async (response) => {
    ...
    // 监听 MEXC API
    if (url.includes('traders/v2') || (url.includes('mexc') && (url.includes('api') || url.includes('copyFutures')))) {
      try {
        ...
        capturedResponses.push({
          ...
          contentCount: data.data && data.data.content ? data.data.content.length : 0,
        })
        
        // 检查响应是否成功
        const isSuccess = status === 200 && (data.success === true || data.code === 0)
        
        if (isSuccess && data.data && data.data.content && Array.isArray(data.data.content)) {
          const content = data.data.content
          if (content.length > 0) {
            console.log(`✅ 从页面响应捕获到 traders/v2 数据: ${content.length} 条`)
            ...
            if (!capturedData || capturedData.length === 0) {
              ...
              const existingIds = new Set(capturedData.map((item) => item.uid || String(item.order)))
              const newRows = content.filter((item) => {
                const id = item.uid || String(item.order)
  ...
  try {
    ...
    console.log('正在访问 MEXC Copy Trading 页面...')
    const targetUrl = 'https://www.mexc.com/futures/copyTrade/home'
    ...
    console.log('')
    console.log('=== 在页面上下文中调用 MEXC API ===')
    const apiUrl = 'https://www.mexc.com/api/platform/futures/copyFutures/api/v1/traders/v2'
    
    // 获取多页数据（最多100条）
    const pageSize = 28 // MEXC 每页28条
    const maxPages = Math.ceil(100 / pageSize) // 4页 = 112条，足够100条
    ...
    const fetchedData = await page.evaluate(async (apiUrl, maxPages, pageSize) => {
      ...
      for (let pageNo = 1; pageNo <= maxPages; pageNo++) {
        try {
          const fetchUrl = `${apiUrl}?condition=%5B%5D&intervalType=NINETY_DAYS&limit=${pageSize}&orderBy=ROI&page=${pageNo}`
          ...
          if (res.ok) {
            ...
            logs.push(`第 ${pageNo} 页 success: ${data.success}, code: ${data.code}`)
            
            if ((data.success === true || data.code === 0) && data.data && data.data.content && Array.isArray(data.data.content)) {
              const rowsCount = data.data.content.length
              allRows.push(...data.data.content)
              logs.push(`✅ 第 ${pageNo} 页: 获取到 ${rowsCount} 条，累计 ${allRows.length} 条`)
              
              // 如果没有更多数据或已获取足够数据，停止
              if (rowsCount < pageSize || allRows.length >= 100) {
                logs.push(`停止获取: 已获取=${allRows.length}, 当前页=${rowsCount}`)
                break
              }
            } else {
    ...
    if (apiRows && apiRows.length > 0) {
      console.log(`✅ 从 API 获取到 ${apiRows.length} 条 MEXC 数据`)
...
async function importToSupabase(normalizedData) {
  ...
  console.log(`完成！共导入 ${sortedData.length} 条 MEXC 90天ROI Top 100 交易员数据`)
}
...
async function main() {
  try {
    ...
    if (jsonPath) {
      ...
      // 没有提供文件路径，尝试自动加载所有 mexc JSON 文件
      const backupDir = join(process.cwd(), 'data', 'backup')
      console.log('自动查找 MEXC JSON 文件...')
      
      if (existsSync(backupDir)) {
        const files = readdirSync(backupDir)
        const mexcFiles = files.filter(f => 
          f.includes('mexc') && f.endsWith('.json')
        ).sort()
        
        console.log(`找到 ${mexcFiles.length} 个 MEXC JSON 文件`)
        
        for (const file of mexcFiles) {
      ...
      if (allRawData.length === 0) {
        ...
        allRawData = await fetchMexc90dRoi()
      }
    }
    ...
    // 保存原始数据到备份目录
    const backupDir = join(process.cwd(), 'data', 'backup')
    const backupPath = join(backupDir, `mexc_90d_raw_${Date.now()}.json`)
    try {
