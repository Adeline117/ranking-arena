...
async function checkSources() {
  const { data } = await supabase.from('trader_snapshots').select('source, arena_score, roi')
  ...
  console.log(
    'Source'.padEnd(20) + 'Total'.padStart(8) + 'Arena Score'.padStart(14) + 'ROI'.padStart(10)
  )
    ...
    .forEach(([source, s]) => {
      console.log(
        source.padEnd(20) +
          ...
          String(s.withScore).padStart(14) +
          String(s.withRoi).padStart(10)
      )
    })
}
...
async function checkAll() {
  // --- trader_scores table ---
  const { data: scores, error } = await supabase.from('trader_scores').select('source')
  ...
  const { data: sources } = await supabase.from('trader_sources').select('source')
  ...
  for (const source of specificSources) {
    ...
    if (data && data.length > 0) {
      data.forEach((t, i) => {
        console.log(
          '  ' + (i + 1) + '. ' + t.season_id + ': ROI ' + t.roi + '%, Score ' + t.arena_score
        )
      })
    }
  ...
  // --- full source count ---
  const { data: all } = await supabase.from('trader_snapshots').select('source')
...
async function checkTop(n = 3) {
  ...
  for (const source of DEFAULT_SOURCES) {
    ...
    data.forEach((t, i) => {
      const id =
        t.source_trader_id?.length > 20
          ? t.source_trader_id.slice(0, 17) + '...'
          : t.source_trader_id
