...
async function main() {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .select(
        'id, platform, trader_key, window, roi_pct, pnl_usd, win_rate, max_drawdown, sharpe_ratio'
      )
  ...
  console.log(
    `Null win_rate: ${nullWr.length}, null max_drawdown: ${nullMdd.length}, null sharpe: ${nullSharpe.length}`
  )
  ...
  if (uniqueMddTraders.length > 0) {
    ...
    for (let i = 0; i < Math.min(uniqueMddTraders.length, 2000); i += 50) {
      ...
      if ((i + 50) % 200 === 0)
        console.log(
          `  equity curve MDD: ${ecMddFilled} computed (${i + 50}/${Math.min(uniqueMddTraders.length, 2000)} checked)`
        )
    }
