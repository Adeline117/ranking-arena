...
- webhook 使用可重试状态机；业务处理失败不会再被误记为“已处理”。但 entitlement writer 尚未全部切到新身份完整 RPC，旧 direct write、退款和定时 reconciliation 仍是 `181840` 前的硬阻断。
...
3. 在 PostgreSQL 17 对抗测试、静态契约、类型检查和迁移 runner 全绿后，只应用 additive `181830` PREDEPLOY；不得同时应用 `181840`。
...
7. 只有 readiness 精确为 ready 且同一待发布 SHA 的 canary 全绿时，才单独应用 `181840` POSTDEPLOY，撤销旧 RPC 并启用数据库 direct-DML guard。此后不能靠回滚旧 app 恢复。
