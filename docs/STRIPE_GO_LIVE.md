...
也不替代后续 live-mode owner gate。2026-07-21 的可回滚切换状态为：

- 旧 test endpoint `we_1TtMgDCL6ewruupgKXyNH9Zf` 仍是唯一 enabled writer；它使用账户默认
  API 版本并包含 legacy refund 事件，因此 readiness 仍应 fail closed。
- 新 test endpoint `we_1TvnrtCL6ewruupgh3pNXkfo` 已用
  `2026-04-22.dahlia` 和本 runbook 的精确 15 个事件创建，canonical URL 已核对，但保持
  `disabled`；密钥部署前不得启用。
- 新 endpoint 的 signing secret 只以 Sensitive Production 变量
  ...
  打印两者的值。
- 部署双密钥实例并完成新 endpoint 的定向重放 canary 后，才允许启用新 endpoint；确认旧
  endpoint 没有 pending delivery 后再停用旧 endpoint。test-mode 反向 primary/previous 只是
  sandbox 过渡态，`stripe-readiness` 必须保留 cutover warning，且不能据此关闭免费 promo。

启用新 endpoint 前的回滚是删除 `STRIPE_WEBHOOK_SECRET_PREVIOUS` 并删除仍 disabled 的新
endpoint；启用后的回滚顺序是先重新启用旧 endpoint，再停用新 endpoint。禁止出现两个都
disabled 的窗口，也禁止在两个 endpoint 同时 enabled 时运行正式 readiness。
