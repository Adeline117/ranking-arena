...
**读漂移 19/19 全清零**（competitions 也修了——写路径 insert 证实真列名 title/start_at/prize_pool_cents/
platform/current_value,读用别名对齐,非猜）。`STRICT_READS=1` 已在 CI 启用（写+读双硬门,锁死不回退）。
competitions 剩的只是**上线与否**的产品决定(代码已不 broken)。
