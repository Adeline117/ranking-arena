...
| 项                                  | 状态 / 为何                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 首页 ISR 失效                       | **修复已部署但不充分**(getHeroStats build-guard 36dfe8135 已上线 17 commit 前)。生产仍 private/no-store/MISS → getHeroStats 是必要非充分,另有 dynamic 源。grep 诊断已穷尽:page.tsx(仅 revalidate=300)/root layout(非 async 无 dynamic API)/TopNav(server 干净)/getInitialTraders+getHeroStats(均已 build-guard)全清,仍 dynamic。**定位真凶需 `next build` 的动态原因报告**(Route / 为何 ƒ),而本机=生产 worker 全量 build 有 OOM 饿死 ingest 风险 → **必须在 CI/preview 环境跑 build 二分**,不本地。性质=perf/成本(边缘缓存),非 correctness(页面正常,数据 Redis 兜底);当前流量无碍,100x 上线峰值才放大函数调用/成本 |
