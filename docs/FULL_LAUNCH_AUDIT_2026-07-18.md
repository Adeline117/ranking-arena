...
1. 五个 predeploy migration 已进入生产 exact ledger；follow composite postdeploy
   必须等目标应用 serving 后再提交；
2. 生产健康端点必须返回本分支精确 SHA；
3. 生产浏览器和数据验收必须通过。
...
| P0-02 | Following 只按 trader id 关联，跨交易所相同 id 可关注错账户或互相覆盖，属于用户关系数据错误。                                                                                              | 关注身份统一为 `(source, exchange_trader_id)`；API、客户端、计数、digest、activity、alert 与 DB composite identity 同步。                                                                                                                                                                                                                           | ✅代码；✅计数/activity predeploy；⏳composite postdeploy/生产应用。                                             |
...
| P0-05 | gTrade/Bitfinex 没有可证明的 ROI 排名基础却仍处于 serving 配置；一旦产生错误 rank 会污染公开榜单。                                                                                         | 保留采集为 active/shadow，但 fail-closed 禁止公开排名；migration 在发现真实 ROI 或公开 rank 时直接失败，避免静默覆盖真数据。                                                                                                                                                                                                                        | ✅🧪；✅生产 migration exact；⏳生产应用。                                                                       |
...
**P0 判定结论**：本分支已修复或隔离已知网页资金/核心数据 P0，但在 migration 和
生产 SHA 验证前都只是“代码完成”，不是“线上清零”。另外 P0-01 的公开合约仍可被直调，
owner 未把 cap 改为 0、P0-08 未完成凭据轮换前，不能宣称“所有 P0 已清零”。
...
| DB ledger    | 五个新 predeploy 全部 exact；composite follow postdeploy 只在目标 app 已 serving 后执行。    |
...
- Production commit：⏳尚未部署本分支；部署后必须由 `/api/health.commit` 精确证明。
- Migration status：✅45/45 predeploy exact；✅既有 social-edge postdeploy exact；
  ⏳`20260717120000` composite identity missing；5/5 concurrent recovery、
  3/3 transactional recovery exact；1 项明确 superseded。
- Production DB smoke：PostgREST `14.5`；freshness `84` 行；新 follower RPC 在
  ...
  `9,397` traders。
- Full Jest：✅678 suites、7,363 tests passed；2 suites / 17 tests 明确 skipped。
...
- Serving QA：⏳最终生产部署后执行。
- Browser QA：⏳最终生产 desktop/mobile/axe/button/core-flow。
- Slow-network first paint：✅本地最终三栏壳与截图；⏳最终生产 slow-network 复验。
- Open owner gates：CopyTrading v1 cap `100 → 0`、生产 DB 密码轮换、Stripe live、
  OKX provider 容量、Sentry sourcemap、Auth 邮件 canary、restore drill。
