...
1. 四个 launch migration 必须进入生产 ledger；
...
| P0-02 | Following 只按 trader id 关联，跨交易所相同 id 可关注错账户或互相覆盖，属于用户关系数据错误。                                                                                              | 关注身份统一为 `(source, exchange_trader_id)`；API、客户端、DB composite identity migration 同步。                                                                                                                                                                                                                                                  | ✅代码；⏳migration/生产。                                                                                       |
...
| P0-05 | gTrade/Bitfinex 没有可证明的 ROI 排名基础却仍处于 serving 配置；一旦产生错误 rank 会污染公开榜单。                                                                                         | 保留采集为 active/shadow，但 fail-closed 禁止公开排名；migration 在发现真实 ROI 或公开 rank 时直接失败，避免静默覆盖真数据。                                                                                                                                                                                                                        | ✅🧪；⏳migration/生产。                                                                                         |
...
| 数据水位          | 旧 API 用全局 score compute time 冒充来源更新时间，无法指出 Binance/Bybit 等单源陈旧。                                                                                  | 新增逐 source/window 水位表；仅在无错误发布后前移；缺失、未来时间、失败批次均 fail-closed。✅代码，⏳migration/生产                                       |
...
| Hero 事实         | “44 exchanges”及旧 exchange count 文案不真实；代码把 source board 当公司数。                                                                                            | 改为 live ranking source boards；API 保留 deprecated `exchangeCount` 但注释真实语义；RPC 用活跃榜源计数。✅代码，⏳migration                              |
...
| 90D 当前公开交易员       |  约 9.2k | 会随 cron 变化，部署后以 API 实数为准。                                        |
...
| DB ledger    | 四个 migration version 全部 recorded；postdeploy 只在目标 app 已 serving 后执行。            |
