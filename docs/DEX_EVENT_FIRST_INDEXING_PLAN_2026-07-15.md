...
- Hyperliquid 已有纯 HTTP board/profile/positions/fills 路径；GMX 使用协议子图；gTrade 使用
  leaderboard + trades table。它们已经是协议级实现，不应为统一技术栈而重写；但当前实现有
  必须先量化的边界：Hyperliquid `userFillsByTime` 约 2,000 fills、请求 fail-soft 且只闭合完整
  round trips；GMX 的 90D 请求实际从 89 天前开始，而数据库 seed 又把 90D 标成 derived；
  gTrade 虽记录 `trades_truncated`，typed publish 尚未用它 fail closed。
...
- **GMX（Arbitrum）**：以现有 Squids/API 结果为对照，事件 decoder 必须包含 position、fee、
  ...
  不能作为事件管线的真值基线。
- **gTrade**：先把现有 `trades_truncated` 接入 typed publish fail-closed 门，再补完整 trade
  lifecycle、费用和截断证据，之后才决定是否取代现 trades table。
