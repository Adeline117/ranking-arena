...
candidate 必须由版本化的宽匹配器先于 decoder 独立产出，例如已登记协议地址/程序命中的成功交易；
不得用“已经成功解码的事件”反过来充当自己的分母。未知 candidate 保留原始定位和 reason。
...
- [`EXCHANGE_FIELD_MAPPING.md`](./EXCHANGE_FIELD_MAPPING.md) 已指出 ROI、PnL、win rate、
  ...
  当成定义相同。
- [`ARCHITECTURE.md`](./ARCHITECTURE.md) 记录了总体 RAW/标准化/serving 分层；实际 count-check、
  typed 评分门和 source 发布行为分散在 ingest/publish 代码、迁移和专项文档中，不能把架构图
  当成完整可执行契约。
- Hyperliquid 已有纯 HTTP board/profile/positions/fills 路径；GMX 使用协议子图；gTrade 使用
  ...
  实现有必须先量化的边界：Hyperliquid `userFillsByTime` 约 2,000 fills、请求 fail-soft 且只
  闭合完整 round trips；GMX 的 90D 请求实际从 89 天前开始，而数据库 seed 又把 90D 标成
  derived。gTrade 已于 2026-07-15 升级为 RAW schema v3：冻结 90D `startDate/endDate`、逐页保存
  原始响应、重放校验 cursor/ID/窗口/USD 价格，并且只有 API 明确 exhaustion 后才发布 typed
  7/30/90 指标；失败窗口只保存审计证据，不刷新 stats/cache/freshness。剩余边界是当前 source
  只覆盖 Arbitrum、90D 开窗前已建立且窗内才关闭的仓位无法从有限窗口证明 lifecycle，以及
  地址发现仍来自现有公开名单而非全事件分母。
...
权威。后续在事件管线首个生产 source 上线时，应同步重写或归档该文件。
...
- **原始事实**与**解释口径**分层。decoder 可以升级，原始事件不被覆盖。
...
最小重放信封：`event_uid, chain_id, height, block_hash, parent_hash, block_time, tx_id,
tx_index, record_kind, chain_sub_index, program_or_contract, topic_or_discriminator, raw_payload,
source_provider, ingest_version, ingested_at`。
...
摄取必须确定性去重，并用重复投递测试证明相同链段 checksum 不变。另建追加式
`chain_block_observations(block_hash, status=commit|revert, observation_version, observed_at)`；
canonical view 以最新 observation 选择有效 block，再联接 raw/facts。禁止在 raw 行上原地翻
`canonical` 布尔值，也不能把 `ReplacingMergeTree` 的最终合并当同步正确性保证。

Phase 0 默认实现**自定义 immutable target**：raw 与 block observation 都只追加，reorg 通过新
observation 撤销，受影响 marts 重建。SQD Pipes 内置 ClickHouse target 不能原样接到这套表：
...
- `fact_event_valuations`：保存 `price_source`、`price_timestamp`、`confidence`、
  `methodology_version`、`amount_usd`。USD 估值不是不可变链事实，不能藏进 raw fill 后失去
  价格版本。
...
派生。协议升级使用新 `protocol_version`，禁止就地改变旧事件含义。
...
- **主页 `/`**：当前数字卡显示 2 小时刷新，但 i18n subtitle 和 `app/page.tsx` SEO 仍写
  “real-time/实时”，这是必须先修的信任漂移。`get_hero_stats.exchange_count` 只是从 90D 有效
  ranking row 的 source slug 取首段去重，不是 `arena.exchanges` 的真实交易所总数；
  `trader_count` 是合格排行账户行数，不是去重钱包或自然人数，同一主体可跨 source 重复。
  OG 图还硬编码 `32+`/“Real-time Rankings”，`ExchangePartners` 又维护一份 22 品牌静态清单，
  三者不是同一分母且可能包含退休/无数据 source。修复后 Hero/SEO/OG/partner strip/tooltip/
  JSON-LD 必须共用可解释的 active-source/platform 口径；事件管线不得把 discovered 地址数
  塞进 Hero。
...
- Pipes SDK 当前仍是 **Beta/alpha package**；2026-07-15 审计版本为
  `@subsquid/pipes@1.0.0-alpha.16`，quickstart 要求 Node.js ≥ 22.15。样板必须锁精确版本与
  runtime，记录 upgrade regression；没有 fork/幂等/退出恢复证据不得升级或进生产。
- SQD gateway 当前要求 API key；“开源”不等于网络服务免费。进入 90D 回填前必须测量
  bytes、请求、限速、重试和实时延迟。
...
| Ponder                     | Arena 限定为单个 EVM 协议的 TypeScript 快速原型 | 这是本项目控制范围，不声称工具只能做单协议；它不统一 Solana，本阶段也不把高吞吐全量事实写进现 Supabase                                     |
...
| Agave + Yellowstone gRPC   | 长期自建 Solana 低延迟流                        | server/plugin 默认 AGPL-3.0-only（clients/proto 为 Apache-2.0），与 Agave 版本强耦合；历史回填另建路径并过法律/容量门                      |

任何候选都必须用同一份 7 天链段和 golden wallet 集合跑 bake-off；比较的是**缺口、可重放、
吞吐、成本和延迟**，不是 GitHub stars。
...
| Gains Network SDK                   | MIT SDK 用于 ABI、类型、公式和 v10+ 版本归一化；官方 backend/live events 作为现路径对账                                                                                                                       | stats subgraph README 明确定位为 Stats & Points，并非不可变 trade lifecycle；仓库根未展示许可证，不 fork 为真相。事件管线按链维护 contract/version manifest，先覆盖 Arbitrum、Base、Polygon、ApeChain；地址发现来自全事件，不来自现 leaderboard topN                                                       |
| GMX 官方 GraphQL                    | 立即修正并使用 `gmx-synthetics-<network>:prod/api/graphql`，覆盖 Arbitrum/Avalanche/Botanix/MegaETH；分页保存 AccountStat、PositionChange、TradeAction、fees/funding 的原始响应                               | 官方文档明确部分 funding 数据是历史 indexer snapshot，不是实时状态；realized PnL、fee、price impact、unrealized 必须分字段。`gmx-subgraph` 根目录未展示许可证；`gmx-synthetics` 在 2026-08-31 前为 BSL 1.1 且不是开源许可证。未经法律/许可证审查不 vendor/fork，Arena 仍需从链上事件建立独立 decoder 真相  |
| Envio HyperIndex / Goldsky Mirror   | 只进入同一 7 天 bake-off 或商业采购比较                                                                                                                                                                       | HyperIndex 官方说明底层 HyperSync 是 proprietary data engine，因此不是完全独立的开源数据面；Goldsky Mirror 是托管 pipeline 能力，不是自托管开源索引器。只有 SLA、成本和可退出性优于 SQD/RPC 时才采购，且必须保留原始归档与 provider 迁移路径                                                               |

协议接入顺序固定为：官方历史/节点或链日志落 RAW → Arena 独立解码与账本 → 官方 API/托管
indexer 对账。任何第三方聚合结果都不能反过来成为自己的完整性分母。
...
- **Hyperliquid**：现有 board + info API 保持主路径。事件管线先 shadow 验证 fills、funding、
  ...
  38,582 是 2026-06-11 的测量值，不是永久人口常量。当前 `userFillsByTime` 约 2,000 fills、
  fail-soft 且只闭合完整 round trip；在补出分页完成证据前，90D 自算必须保持 partial 并披露
  采样与资金流误差。
- **GMX（Arbitrum）**：以现有 Squids/API 结果为对照，事件 decoder 必须包含 position、fee、
  funding、liquidation，不用 swap-only 账本替代 perp 语义。现代码的 90D 请求从 89 天前开始，
  但 DB seed 又把 90D 标为 derived；先统一窗口和 native/derived 契约并补回归测试，否则 GMX
  不能作为事件管线的真值基线。
- **gTrade**：现有 cursor history 的 schema v3 覆盖门继续作为生产原生入口；事件管线不重复
  实现这层分页。下一步用官方 SDK/ABI 解释协议版本，按 Arbitrum、Base、Polygon、ApeChain
  分别回放 open/increase/decrease/partial-close/close/liquidation 与费用，建立全事件地址分母；
  先解决开窗前仓位 anchor，再与 v3 snapshot 做逐钱包 shadow diff。未覆盖链、缺 opening event
  或 lifecycle 未闭合时保持 partial，不用当前 Arbitrum 公开名单冒充 gTrade 全网交易员宇宙。
...
- [ ] 生成 `dex_protocol_manifest`：两条链的协议/程序/合约、版本、高度和 decoder owner。
- [ ] 固定 100 个 golden wallets：top 40、随机 40、异常/高频 20；固定 golden tx 清单。
- [ ] 用 SQD 拉同一 7 天 BSC 与 Solana 样本，记录完整性、延迟、bytes、费用投影。
- [ ] 建本地/隔离 ClickHouse DDL 与 dbt skeleton，不连接生产 serving。
...
| disaster       | provider 断流、ClickHouse 只读/丢实例、decoder bad deploy、Postgres publish 失败                       |

CI 只使用固定小链段和 fixtures；生产 90D shadow 在独立 job 跑。任何 diff 报告必须保存原始
tx 定位信息，不能只输出汇总百分比。
...
- 生产启用付费 provider/Cloud 前，记录报价、预算 owner 和 hard monthly alert；超预算 80% 告警，
  100% 禁止自动扩容/回填。
- 按协议地址过滤原始流；不为了未来可能用到而永久保存整条链所有无关事件。

### 11.3 建议保留

- ClickHouse raw replay envelope：30 天热存；规范事实：至少 120 天热存，覆盖 90D 重算余量。
- R2/对象存储 Parquet：按链/日期归档 **raw replay envelope + 规范事实 + manifest**，至少保留
  1 年。只存已解码事实无法修复旧 decoder 错误；先做一次恢复和重解码演练再称其为备份。
- daily marts、发布 run、quality evidence：长期保留，用于历史排名解释。
- 删除/TTL 不得早于对应 publish run 的可重放范围；改保留期需先证明 SQD 可回放且成本可接受。
...
4. 若降为 partial，同一事务清空对应 typed score inputs；随后重算排行并失效缓存。按
   `(height range, decoder_manifest_sha, model_sha)` 重放，不做无边界全库 UPDATE。
