...
不得用“已经成功解码的事件”反过来充当自己的分母。未知 candidate 保留公开 block/tx 定位、
定位哈希和固定 reason，不保留 provider body。
...
- [`ARCHITECTURE.md`](./ARCHITECTURE.md) 与 `lib/ingest/raw.ts` 记录了现有通用
  ...
  清理，且当前没有 bucket lifecycle 兜底，所以 provider body 可能无限保留。这个既有实现不是
  新增 DEX evidence/acquisition artifact 的授权；gTrade 等 legacy 路径必须单独做 owner
  授权、quarantine/manual-retention 和迁移审计。实际 count-check、typed 评分门和 source 发布
  行为分散在 ingest/publish 代码、迁移和专项文档中，不能把架构图当成完整可执行契约。
...
#### `acquisition_batch_observations` / `acquisition_page_observations`
...
每页只写：
`page_observation_id, batch_id, page_ordinal, request_params_sha256,
input_cursor_commitment, output_cursor_commitment, public_start_height, public_end_height,
public_start_time, public_end_time, transport_status, content_length, content_sha256, item_count,
...
payload_sha256, payload_byte_length, batch_id, page_observation_id, page_item_ordinal,
decoder_manifest_sha, normalized_fact_set_sha256, verification_state, ingest_version,
ingested_at`。`payload_sha256` 只绑定当次内存输入，不对应已保存 blob；表内不得出现
request/response body、normalized JSON body、header、credential URL/query、cookie/token 或
provider error body。
...
另建追加式 `chain_block_observations(chain_id, height, block_hash, parent_hash,
status=commit|revert, observed_finality, source_provider, source_endpoint_identity_sha256,
source_independence_group, batch_id, page_observation_id, page_item_ordinal, observation_reason,
observation_version, observed_at)`。canonical view 必须在同一 `(chain_id,height)` 内按经过验证的
finality 与最新 observation 选择有效 block，再联接 observations/facts；独立差分只能比较不同
`source_independence_group`，不能把同一 archive/operator 的两个客户端算双源。禁止在 observation
行上原地翻 `canonical` 布尔值，也不能把 `ReplacingMergeTree` 的最终合并当同步正确性保证。
...
`batch_id/page_observation_id/page_item_ordinal` 传递页级 provenance。协议升级使用新
`protocol_version`，禁止就地改变旧事件含义。
...
- [x] 补 BSC/Solana `dex_protocol_manifest`：协议/程序/合约、版本、生效高度、证据来源和
...
- [ ] 从 facts + observation metadata + manifest/checksum ledger 做空 ClickHouse 集群恢复，
      验证 §11.4 的 checksum、active-pointer、RPO/RTO 硬门。
...
在隔离的空 ClickHouse 集群完成：

1. 只从对象存储的规范事实、batch/page/event/block observation metadata、manifest 和 checksum
   ledger 恢复；禁止读取生产 ClickHouse 或 provider body。
2. 逐分区核对 row count、首末高度、重复主键、gap、fact-set SHA、batch/page hash chain 和
   decoder/manifest SHA；恢复后的 checksum 必须与备份源完全相等。
3. 重建 marts 后，对账 Postgres `dex_publish_runs` 与 `dex_source_active_runs`：active pointer
   必须指向已完整恢复且仍通过质量门的 run。缺 run、hash 漂移或悬空 pointer 必须 fail closed，
   继续服务 last-known-good/stale 快照，禁止用空结果或半恢复数据覆盖生产。
