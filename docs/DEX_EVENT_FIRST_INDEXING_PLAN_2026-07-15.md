...
>
> 证据刷新：2026-07-16。当前代码、只读 census、桌面截图与协议官方资料冲突时，以可重放
> RAW、当前 fixture/代码契约和官方链/合约状态为准；旧页面文案只保留为带日期的 UI 证据。
>
...
- Hyperliquid 已有纯 HTTP board/profile/positions/fills 路径；GMX 使用协议子图；gTrade 使用
  ...
  冻结 90D `startDate/endDate`、逐页保存原始响应、重放校验 cursor/ID/窗口/USD 价格，并且只有
  API 明确 exhaustion 后才发布 typed 7/30/90 指标；失败窗口只保存审计证据，不刷新
  stats/cache/freshness。其生产 source 仍只覆盖 Arbitrum，开窗前仓位无法从有限窗口证明
  lifecycle，地址发现仍来自公开 Top-25 榜单而非全事件分母。
...
| Envio HyperIndex           | 仅法务批准后的 PoC/shadow                  | 官方许可说明不是 OSI 开源，generator/generated code 有用途限制，HyperSync 也不是独立开源数据面；不得以“开源索引器”名义进入核心链路              |
