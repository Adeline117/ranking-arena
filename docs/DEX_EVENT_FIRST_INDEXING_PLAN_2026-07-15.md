...
| Carbon                       | Solana decoder/processor 与数据源适配器 PoC | MIT Rust；支持 nested CPI、IDL 生成、Geyser/RPC/历史/快照 datasource。它是可复用的解码框架，不是独立数据真相、finality 证明或 BSC 解码器；同一交易仍须与 Arena RAW/evidence contract 对接                                                                         |
...
- [ ] 补 BSC/Solana `dex_protocol_manifest`：协议/程序/合约、版本、生效高度、证据来源和
      decoder owner；不能把 GMX/gTrade deployment fixture 冒充这两条通用链的 manifest。
